...
class MonsterInsights_eCommerce_RCP_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	private function track_purchase_ua( $order ) {
		...
		$atts = array(
			...
			'ev'  => round( $order->amount * 100 ),                                          // Event Value
			'cos' => 2,                                                                      // Checkout Step
			'pa'  => 'purchase',                                                             // Product Action
			'cid' => monsterinsights_get_client_id( $order->id ),                           // GA Client ID
			'ti'  => $order->id,                                                            // Transaction ID
			'ta'  => null,                                                                   // Affiliation
			'tr'  => $order->subtotal,                                                       // Revenue
			'tt'  => '',                                                                     // Taxes
			'ts'  => '',                                                                     // Shipping
			'tcc' => $discount,                                                              // Discount code
			'cu'  => rcp_get_currency(),                                                     // Currency
		);
	...
	private function track_purchase_v4( $order ) {
		...
		$args = array(
			'events'    => $events,
			'client_id' => monsterinsights_get_client_id( $order->id ),
		);
