...
class MonsterInsights_eCommerce_LifterLMS_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	private function track_purchase_ua( $order, $txn ) {
	...
	private function track_purchase_v4( $order, $txn ) {
		...
		$args = array(
			...
			'client_id' => monsterinsights_get_client_id( $order->get( 'id' ) ),
		);
	...
	public function add_order( $txn ) {
		...
		$is_in_ga = get_post_meta( $txn_id, '_monsterinsights_is_in_ga', true );
		...
		$this->track_purchase_ua( $order, $txn );
		$this->track_purchase_v4( $order, $txn );

		update_post_meta( $txn_id, '_monsterinsights_is_in_ga', 'yes' );
	}
	...
	private function track_refund_ua( $order, $txn ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
	...
	private function track_refund_v4( $order, $txn ) {
		if ( ! function_exists( 'monsterinsights_get_v4_id_to_output' ) ||
		     ! function_exists( 'monsterinsights_mp_collect_v4' ) ||
		     ! monsterinsights_get_v4_id_to_output()
		) {
	...
	public function remove_order( $txn ) {
		...
		$is_in_ga = get_post_meta( $txn_id, '_monsterinsights_refund_is_in_ga', true );
		...
		$this->track_refund_ua( $order, $txn );
		$this->track_refund_v4( $order, $txn );
