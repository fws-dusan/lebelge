...
class MonsterInsights_Frontend_Custom_Dimensions
{
    ...
    public function __construct()
    {
        ...
        add_filter('monsterinsights_frontend_tracking_options_analytics_before_pageview', array(
            $this,
            'output_custom_dimensions_analytics',
        ));

        add_filter('monsterinsights_frontend_tracking_options_gtag_before_pageview', array(
            $this,
            'output_custom_dimensions_gtag',
        ), 10, 2);

    }

    public function output_custom_dimensions_analytics($options)
    {
        return $this->output_custom_dimensions($options, 'analytics');
    }

    public function output_custom_dimensions_gtag($options, $type = 'ua')
    {
        if ($type === 'ua') {
            return $this->output_custom_dimensions($options, 'gtag');
        }

        return $this->output_custom_dimensions_v4($options);
    }

    private function output_custom_dimensions_v4($options)
    {
