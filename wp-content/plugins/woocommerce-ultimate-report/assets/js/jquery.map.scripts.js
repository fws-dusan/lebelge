	 ...
	 $('#map1').vectorMap({
		...
		onRegionTipShow: function(e, el, code){
			if (!data_total[code]) {
				...
				el.html(ic_ajax_object.country+": " +el.html()+' <br />'+ic_ajax_object.order_total+': '+ic_ajax_object.currency_symbol+' '+data_total[code]+'<br />'+ic_ajax_object.order_count+': ' + data_count[code]);
			}
		}
	  });
}
jQuery(document).ready(function($) {
	...
	jQuery(function(){
		...
		columns['start_date'] 			= jQuery("#start_date").val();
		columns['end_date'] 			= jQuery("#end_date").val();
