...
var button_customize_columns_label = ic_ajax_object.customize_column;
var defaultOpen = "";
...
function pull_product_dropdonw(){
	...
	$('.ajax_progress').html(ic_ajax_object.please_wait).fadeIn();
	...
	$.ajax({
			...
			success:function(data) {								
					...
					$('.ajax_progress').html(ic_ajax_object.please_wait).fadeOut();
					$(".form_process").hide();
...
function checkbox_check(do_action_type){
	var checked_count = jQuery('.'+do_action_type+' input:checked').length;
...
function hidePopup() {
	if (popup_open == true) {
        jQuery(".popup_mask").fadeOut("slow");
...
function back_to_detail(){
	...
	jQuery('.hide_for_print').show();
	jQuery('.hide_for_print').show();
	jQuery('.hide_for_print').show();
	jQuery('.search_for_print_block').hide();
}
...
/*	$("#ga_ao_start_date").datepicker({
	...
	*/
	...
	if(search_content.length == 0){
		$('div.search_report_content').html(ic_ajax_object.please_wait);
	...
	$(document).on('click','input.search_for_print',  function(){
			...
			$('.hide_for_print').hide();
			$('.hide_for_print').hide();
			$('.search_for_print_block').show().html(ic_ajax_object.please_wait);
	...
	jQuery('#button_customize_columns').attr('data-show_columns','hide');
	$('#button_customize_columns').click(function(){
		
		var v = jQuery(this).val();
		var show_columns = jQuery(this).attr('data-show_columns');
		if(show_columns == 'hide'){
			jQuery(this).val(ic_ajax_object.hide_column_view);
			if(jQuery("#detail_view").is(':checked')){
				jQuery(".search_by_details_fields").show();
				jQuery(".search_by_normal_fields").hide();
			}else{
			...
			jQuery(this).attr('data-show_columns','show');
		}else{
			jQuery(".search_by_normal_fields").hide();
			jQuery(".search_by_details_fields").hide();
			jQuery(this).val(ic_ajax_object.customize_column);
			jQuery(this).attr('data-show_columns','hide');
		}
	}).val(ic_ajax_object.customize_column);
	...
	var pre_active_li = $(".responsive-tabs").find('li.active');
	var pre_active_tab_id = pre_active_li.find('a').attr('data-tab');						
	$(pre_active_tab_id).hide();
	pre_active_li.removeClass('active');
	
	var parent_tab = jQuery('.responsive-tabs-content').find('a.nav-tab-active').attr('data-parent_tab');
	jQuery('#button_'+parent_tab).addClass('active');
	jQuery('#tab_'+parent_tab).show();
