...
if (!class_exists('	')) {
	...
	class IC_Commerce_Ultimate_Woocommerce_Report_New_Repeat_Customer  extends  IC_Commerce_Ultimate_Woocommerce_Report_Functions{
		...
		*/
		function init(){
             ...
             <div class="responsive-menu"><a href="#" id="menu-icon"></a></div>
                <?php            	
                   foreach ( $page_titles as $key => $value ) {
                        echo '<a href="'.admin_url( 'admin.php?page='.$page.'&customer_type=' . urlencode( $key ) ).'" class="nav-tab ';
                        if ( $customer_type == $key ) echo 'nav-tab-active';
            	...
            	<div class="content">
                	 <div class="search_report_form">
                           ...
                           <div class="form-table">
								...
								<input type="<?php echo $input_type; ?>" name="do_action_type" value="new_repeat_customer" />
		 ...
		 <div class="search_report_content hide_for_print autoload_<?php echo $onload_search;?>">
            ...
            <style type="text/css">
            	.iccommercepluginwrap .search_report_content .widefat thead th.order_total{ text-align:right;}
            </style>
			<?php
		}
		...
		*/
		function get_customer($billing_email ='', $start_date  ,$end_date ,$type = "row"){
			...
			$query .= " LEFT JOIN $wpdb->postmeta AS billing_email ON billing_email.post_id = posts.ID";
			$query .= " LEFT JOIN $wpdb->postmeta AS order_total ON order_total.post_id = posts.ID";
			
			$query .= " LEFT JOIN $wpdb->postmeta AS billing_first_name ON billing_first_name.post_id = posts.ID";
			$query .= " LEFT JOIN $wpdb->postmeta AS billing_last_name ON billing_last_name.post_id = posts.ID";
		...
		*/
		function get_grid(){
			...
			if (count($customer_row)>0): ?> 
                <form name="frm_export_new_repeat_customer" id="frm_export_new_repeat_customer">
				...
				<div class="top_buttons">
                	<div class="RegisterDetailExport">
                    	<input type="submit" name="<?php echo $this->constants['plugin_key'].'_new_repeat_customer_export_csv';?>" id="export_customer_product" value="Export" class="onformprocess" />  
                    </div>
            	...
            	<?php
                	$admin_url = admin_url('admin.php')."?page=icwoocommerceultimatereport_details_page&detail_view=no&start_date={$start_date}&end_date={$end_date}&order_status=-1&onload_search=yes&";
				?>
                ...
                <?php if (count($customer_row)>0): ?>                 	
					<?php foreach($customer_row as $key=>$value): ?>
                        ...
                        <?php $billing_email = $value->billing_email;?>
                        <tr class="<?php echo $alternate."row_".$key;?>">
                            <td><?php echo $value->billing_first_name; ?></td>
                            <td><?php echo $value->billing_last_name; ?></td>
                            <td><a href="<?php echo $admin_url."&paid_customer=".$billing_email;?>"><?php echo $value->billing_email; ?></a></td>
                            <td class="order_count amount"><a href="<?php echo $admin_url."&paid_customer=".$billing_email;?>"><?php echo $value->order_count; ?></a></td>
                            <td class="order_total amount"><?php echo $this->get_woo_price( $value->order_total); ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
					<tr>
		...
		*/
		function  export_customer(){
			...
			$FileName = "new-repeat-customer-report"."-".$today.".csv";	
			...
			$export_file_name 		= "new-repeat-customer-report";
