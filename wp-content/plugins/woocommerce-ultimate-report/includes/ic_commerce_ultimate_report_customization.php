...
if ( ! class_exists( 'IC_Commerce_Ultimate_Woocommerce_Report_Customization' ) ) {
	require_once('ic_commerce_ultimate_report_functions.php');
	class IC_Commerce_Ultimate_Woocommerce_Report_Customization extends IC_Commerce_Ultimate_Woocommerce_Report_Functions{
		...
		*/
		...
		/*
		* init 
		* Include all filter here, set request into constant variable 
		*/
		function init(){
			...
			if($admin_page == "icwoocommerceultimatereport_details_page"){
				...
				if($detail_view == 'yes'){
					...
					add_filter("ic_commerce_details_view_data_items", 							array($this, "join_order_itemmeta"),								25,	3);
					add_filter("ic_commerce_details_view_data_items", 							array($this, "get_ic_commerce_details_view_data_items"),			27,	3);
				}
				
			}
			
			if($admin_page == "icwoocommerceultimatereport_variation_page"){
			...
			/*if($admin_page == "icwoocommerceultimatereport_tax_report_page"){
				add_action("ic_commerce_variation_page_above_show_fields",			array($this, "ic_commerce_variation_page_above_show_fields"),20);
			}*/
			
		}//End Method
		/**
		* ic_commerce_detail_page_search_form_before_order_by
		*
		* 
		*
		* @param object $this_ 
		...
		*/
		function ic_commerce_detail_page_search_form_before_order_by($this_){
			...
			if($page == "icwoocommerceultimatereport_details_page"){
            	...
            	<?php
			}//End icwoocommerceultimatereport_details_page;
		}
		
		/**
		* ic_commerce_details_page_footer_area
		*
		* Get footer details
		*
		*
		...
		*/
		...
		/**
		* get_ic_commerce_normal_view_join_query
		*
		*
		* @param array $sql 
		* @param array $request 
		...
		*/
		function get_ic_commerce_normal_view_join_query( $sql = array(), $request = array()){ 
			...
			if($city_code and $city_code != -1){
				$sql .= " LEFT JOIN {$wpdb->postmeta} as billing_city ON billing_city.post_id=posts.ID";
			}
		...
		/**
		* get_ic_commerce_normal_view_where_query
		*
		*
		* @param array $sql 
		* @param array $request 
		...
		* get_state
		*
		* Get state name by state code and country code
		*
		* @param string $cc 
		* @param string $st 
		*
		...
		*/
		function get_city(){
			...
			$sql = "SELECT 
					...
					FROM `{$wpdb->posts}` AS posts
					LEFT JOIN {$wpdb->postmeta} as billing_city ON billing_city.post_id=posts.ID
		...
		*/
		...
		/**
		* ic_commerce_variation_page_above_show_fields
		*
		*
		* @param object $this_ 
		*
		...
		*/
		...
		/**
		* get_ic_commerce_report_page_default_items
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		...
		/**
		* get_ic_commerce_custom_all_category_query
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		...
		/**
		* get_ic_commerce_custom_all_billing_state_query
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		function get_ic_commerce_custom_all_billing_state_query(		$type = 'limit_row', $columns = array(), $report_name = "", $this_filter = NULL){
			...
			if(!isset($this_filter->items_query)){
				...
				,postmeta3.meta_value AS 				'billing_country_name'
		...
		/**
		* get_ic_commerce_custom_all_billing_city_query
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		function get_ic_commerce_custom_all_billing_city_query(		$type = 'limit_row', $columns = array(), $report_name = "", $this_filter = NULL){
			...
			if(!isset($this_filter->items_query)){
				...
				LEFT JOIN  {$wpdb->postmeta} as postmeta2 ON postmeta2.post_id=posts.ID
				LEFT JOIN  {$wpdb->postmeta} as postmeta3 ON postmeta3.post_id=posts.ID
				LEFT JOIN  {$wpdb->postmeta} as postmeta4 ON postmeta4.post_id=posts.ID";
		...
		/**
		* get_ic_commerce_custom_all_tax_items_query
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		function get_ic_commerce_custom_all_tax_items_query(			$type = 'limit_row', $columns = array(), $report_name = "", $this_filter = NULL ){
			...
			if(!isset($this_filter->items_query)){
				...
				$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta2 ON postmeta2.post_id=woocommerce_order_items.order_id";
				...
				$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta3 ON postmeta3.post_id=woocommerce_order_items.order_id";
				$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta4 ON postmeta4.post_id=woocommerce_order_items.order_id";
				
				if($country_code and $country_code != '-1')	$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta5 ON postmeta5.post_id=posts.ID";
		...
		/**
		* get_ic_commerce_custom_all_customer_buy_products_query
		*
		* @param array $rows  
		* @param string $type  
		* @param array $columns  
		...
		*/
		function get_ic_commerce_custom_all_customer_buy_products_query($type = 'limit_row', $columns = array(), $report_name = "", $this_filter = NULL){
			...
			if(!isset($this_filter->items_query)){
				...
				$sql .= " 	LEFT JOIN  {$wpdb->postmeta} as postmeta_billing_first_name ON postmeta_billing_first_name.post_id		=	woocommerce_order_items.order_id";
				$sql .= " 	LEFT JOIN  {$wpdb->postmeta} as postmeta_billing_last_name ON postmeta_billing_last_name.post_id			=	woocommerce_order_items.order_id";
				$sql .= " 	LEFT JOIN  {$wpdb->postmeta} as postmeta_billing_billing_email ON postmeta_billing_billing_email.post_id	=	woocommerce_order_items.order_id";
				$sql .= " 	LEFT JOIN  {$wpdb->postmeta} as postmeta_customer_user ON postmeta_customer_user.post_id	=	woocommerce_order_items.order_id";
			...
			switch($report_name){
				...
				case "billing_state_page":
					$columns 	= array(
						...
						,"billing_country_name"			=> __("Billing Country", 		'icwoocommerce_textdomains')
						,"order_count"				=> __("Order Count", 			'icwoocommerce_textdomains')
						,"total_amount"				=> __("Amount", 				'icwoocommerce_textdomains')
					);
					
					$billing_or_shipping 						= $this->get_request('billing_or_shipping','billing');
					if(isset($columns['billing_country_name'])) 		$columns['billing_country_name'] 	= $billing_or_shipping == "shipping" ? __( 'Shipping State' , 'icwoocommerce_textdomains') : $columns['billing_country_name'];
		...
		/**
		* get_ic_commerce_report_page_result_columns
		*
		...
		*/
		...
		/**
		* get_ic_commerce_pdf_custom_column_right_alignment
		*
		* @param array $total_columns  
		* @param array $column  
		* @param string $report_name  
		...
		*/
		...
		/**
		* get_ic_commerce_report_page_titles
		*
		...
		*/
		function get_ic_commerce_report_page_titles($page_titles = array(),$report_name = "", $plugin_options = array()){
			...
			$page_titles 				= array(
				...
				,'customer_buy_products_page'	=> __('Customer Wise Products',	'icwoocommerce_textdomains')
		...
		* @return array $order_items
		*/
		function join_order_itemmeta($order_items = array(), $request = array(), $type = "", $page = "", $columns = array()){
			
			
				if(count($order_items)>0){
					$order_item_ids = $this->get_items_id_list($order_items,'order_item_id');				
					$order_itemmeta = $this->get_order_itemmeta($order_item_ids);				
					
					foreach ( $order_items as $key => $order_item ) {
						$order_item_id = isset($order_item->order_item_id) ? $order_item->order_item_id : 0;
						$order_meta 	= isset($order_itemmeta[$order_item_id]) ? $order_itemmeta[$order_item_id] : array();
		...
		/**
		* get_ic_commerce_details_view_data_items
		* @return array $order_items
		*/
		function get_ic_commerce_details_view_data_items($order_items = array(), $request = array(), $type = "", $page = "", $columns = array()){
			
				if(count($order_items)>0){
					$variation_columns = isset($this->constants['variation_columns']) ? $this->constants['variation_columns'] : array();
					foreach ($order_items as $key => $order_item ) {
						foreach($variation_columns as $column_key => $column_label){
							$order_items[$key]->{$column_key} = isset($order_item->{$column_key}) ? ucfirst($order_item->{$column_key}) : '';
						}
					}
				}
