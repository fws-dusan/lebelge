...
if ( ! class_exists( 'IC_Commerce_Ultimate_Woocommerce_Report_Stock_List_report' ) ) {

/*
	 * Class Name IC_Commerce_Ultimate_Woocommerce_Report_Stock_List_report
	 *
	 * Class is used for returning stock list data
	 *	 
	*/

	require_once('ic_commerce_ultimate_report_functions.php');
	class IC_Commerce_Ultimate_Woocommerce_Report_Stock_List_report extends IC_Commerce_Ultimate_Woocommerce_Report_Functions{
					...
					<div class="content">
						<div class="search_report_form">
							...
							<form action="" name="search_order_report" id="search_order_report" method="post">
                               ...
                               <?php do_action('ic_commerce_stock_list_page_searrch_form_bottom',$this,'stock_list');?>
                                ...
                                <div class="form-group">
                                    <div class="FormRow">
                                        ...
                                        <input type="hidden" name="report_name"		id="report_name"		value="<?php echo $this->get_request('report_name','stock_list',true);?>" />
		...
		* This function is used for select downloadable virtual products
		* @param string $output 
		* @return string
		*/
		...
		/*
			* Function Name product_type
			*
			* Returns resulting columns for simple, variable, virtual, downloadable products
			*
			* @param string $terms
			*
			* @param string $output
											...
											foreach($columns as $key => $value):
												...
												switch($key){
													...
													case "variation_stock":													
														...
														$v = (strlen($v)>0) ? $v : (isset($order_item->stock) ? $this->get_stock($order_item->stock) : __("Not Set",'icwoocommerce_textdomains'));
														break;
													case "product_stock":													
														$v = isset($order_item->stock) ? ($this->get_stock($order_item->stock) + 0) : __("Not Set",'icwoocommerce_textdomains');
														break;
													case "product_sku":													
														$v = isset($order_item->sku) ? $this->get_stock($order_item->sku) : __("Not Set",'icwoocommerce_textdomains');
														break;
													case "variation_sku":													
														$v = get_post_meta($order_item->variation_id,"_sku",true);
														$v = (strlen($v)>0) ? ($v + 0) : (isset($order_item->sku ) ? $this->get_stock($order_item->sku) : __("Not Set",'icwoocommerce_textdomains'));
														break;
													case "product_name":
														if($clickeble)
		...
		* This function is used for products multiselect 
		* @param inte $id 
		* @param string $values 
		* @return string
		*/
		...
		* This function is used for columns
		...
		*/
		...
		function ic_commerce_custom_report_normal_query($type = 'total_row'){
					...
					if(!$this->normal_sql_query){
						...
						if($sku_number || ($product_sku and $product_sku != '-1')) $sql .= " LEFT JOIN  {$wpdb->postmeta} as sku 				ON sku.post_id				=posts.ID";
						
						if($product_stock || $txtMinStock || $txtMaxStock || $zero_stock == "yes" || $sort_by == "stock") 		$sql .= " LEFT JOIN  {$wpdb->postmeta} as stock 				ON stock.post_id			=posts.ID";//Modified Change ID 20150309
						...
						$sql = apply_filters('ic_commerce_stock_list_page_join_query',$sql,$request,$type,'',array());
						...
						$sql = apply_filters('ic_commerce_stock_list_page_where_query',$sql,$request,$type,'',array());
		...
		/**
		* get_all_product_sku
		* This function is used for fetching all produuct's SKU
		* @param string $sql 
		* @return int
		*/
		...
		* This function is used for customer by product data
		...
		*/
		...
		/**
		* get_product_type_by_product_id
		* This function is used for product type filter
		* @param string $term_name 
		* @return string
		*/
		...
		/**
		* total_count
		* This function is used for products total count
		* @param string $term_name 
		...
		*/
		...
		/**
		* GetDataGrid
		* This function is used for get data into grid
		*/
		...
		/**
		* grid_columns
		* This function is used for get report columns
		*/
		...
		*/
		...
		/**
		* _get_setting
		* This function is used for stock list setting
		*/
