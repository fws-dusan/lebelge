...
if(!class_exists('IC_Commerce_Ultimate_Woocommerce_Report_Schedule_Mailing_Sales_Status')){	
	require_once('ic_commerce_ultimate_report_functions.php');	
	/*
	 * Class Name IC_Commerce_Ultimate_Woocommerce_Report_Schedule_Mailing_Sales_Status
	 *
	 * Class is used for returning Report of Scheduled Mail Sales Status.
	 *	 
	*/
	class IC_Commerce_Ultimate_Woocommerce_Report_Schedule_Mailing_Sales_Status extends IC_Commerce_Ultimate_Woocommerce_Report_Functions{
			...
			*/
			public function __construct($file, $constants) {
				...
				$this->constants['error_message'] = "";
			}
			
			/**
			* force_schedule_event
			*/
			...
			/**
			* cron_schedule_event
			* This Function is used for Cron Schedule Event.
			*/
			...
			/**
			* doing_wp_cron_schedule_event
			* This Function is used for WP Cron Schedule Event.
			*/
			...
			/**
			* ajax_schedule_event
			* This Function is used for Ajax Schedule Event.
			*/
			//Added 20150219
				...
				if($flag){
					...
					if(isset($this->constants['status_report_emailed'])){
						if($this->constants['status_report_emailed']){
							...
							if($this->constants['error_message']){
								$message .= $this->constants['error_message'];
							}else{
			...
			* This Function is used for Schedule Mail Send.
			*/
			public function ic_woo_schedule_send_email() {
				...
				|| $isset_doing_wp_cron 	==	1 //Added 20150721
					...
					$this->constants['error_message'] .= '<li>'.__("Please select at list one report.",'icwoocommerce_textdomains').'</li>';
					error_log('function Please select at list one report');
					return false;					
				}
				...
				|| $isset_doing_wp_cron 	==	1 //Added 20150721
					...
					if(strlen($email_data)>0){	
						...
						*/
						...
						//$this->constants['error_message'] .= $email_send_to ." - ". $email_from_email;
						...
						if($email_send_to || $email_from_email){							
							...
							error_log('Seding stock status email');
							$result = wp_mail( $to, $subject, $message, $headers); 
							
							if($result){
								$this->constants['success_message'] .= '<li>'.__("Email has been sent successfully.",'icwoocommerce_textdomains').'</li>';
							}else{
								error_log('Getting problem while sending mail');
								$this->constants['error_message'] .= '<li>'.__("Getting problem while sending mail.",'icwoocommerce_textdomains').'</li>';
							}
							
							return $result;
						}else{
							if(!$email_send_to){
								$this->constants['error_message'] .= '<li>'.__("Invalid  \"TO\" email.",'icwoocommerce_textdomains').'</li>';
							}
							
							if(!$email_from_email){
								$this->constants['error_message'] .= '<li>'.__("Invalid  \"FROM EMAIL\"  email.",'icwoocommerce_textdomains').'</li>';
							}
						...
						error_log('Getting problem while sending mail, email content got blank.');
						
						$this->constants['error_message'] .= '<li>'.__("Getting problem while sending mail, email content got blank.",'icwoocommerce_textdomains').'</li>';
					}
				else:
					error_log('Getting problem while sending mail, please select at leat one report.');
					$this->constants['error_message'] .= '<li>'.__("Getting problem while sending mail, please select at leat one report.",'icwoocommerce_textdomains').'</li>';
				endif;
				error_log('-----------------------End------------------------');
				return false;
			}
			...
			/**
			* getEmailData
			* This Function is used to get Email Data.
			...
			*/
			function getEmailData($start_date, $end_date, $title = "Daily",$post_status,$shop_order_status, $report_type = ""){
					...
					if($this->constants['post_order_status_found'] == 0 ){
						...
						$status_sql_query = " AND  posts.post_status NOT IN ('auto-draft','inherit','private')";
					}
					...
					$sql = "SELECT 
					SUM(postmeta.meta_value) AS 'total_sales' FROM {$wpdb->postmeta} as postmeta 
					...
					LEFT JOIN  {$wpdb->postmeta} as postmeta6 ON postmeta6.post_id=woocommerce_order_items.order_id
					LEFT JOIN  {$wpdb->posts} as posts ON posts.ID=woocommerce_order_items.order_id	";
					$sql .= $status_join_query;
					$sql .= " 						 
					WHERE 						
					...
					$sql .= " 
					FROM {$wpdb->prefix}woocommerce_order_items as woocommerce_order_items				
					LEFT JOIN  {$wpdb->postmeta} as postmeta1 ON postmeta1.post_id=woocommerce_order_items.order_id";				
					...
					LEFT JOIN  {$wpdb->posts} as posts ON posts.ID =	shop_order.ID
					";
					$sql .= $status_join_query;
			...
			*/
			...
			/**
			* define_constant
			* This Function is used for Display Logo.
			*/
			...
			* This Function is used to get Total Customer Count.
			*/
			var $total_customer = "";
			...
			/**
			* get_total_today_customer
			* This Function is used to get Today's Total Customer.
			*/
			...
			* @return string
			*/
			function price_($value){
				if(function_exists('wc_price')){
					$v = wc_price($value);
				}elseif(function_exists('woocommerce_price')){
					$v = woocommerce_price($value);
				}else{
					$v = apply_filters( 'icwoocommercegolden_currency_symbol', '&#36;', 'USD').$value;
				}
				return $v;
			}
			...
			*/
			function check_email($check) {
				$expression = "/^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.([a-zA-Z]{2,6})$/";
			...
			*/
			function get_email_string($emails){
				...
				foreach($emails as $key => $value):
					...
					if($this->check_email($e)){
						...
						$this->constants['error_message'] .= "invalid email " . $e;
					}				
				endforeach;
			...
			/**
			* get_ipaddress
			* This Function is used to get IP Address.
			* @return integer
			*/
