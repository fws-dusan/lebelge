...
if ( ! class_exists( 'IC_Commerce_Ultimate_Woocommerce_Report_Grid_Data' ) ) {
		/*
		 * Class Name IC_Commerce_Ultimate_Woocommerce_Report_Grid_Data
		 *
		 * Class is used for display data in Grid 
		 *	 
		*/
	class IC_Commerce_Ultimate_Woocommerce_Report_Grid_Data{
		...
		*/
		...
		/**
		* create_grid_items
		* This function is used for Create Grid Items
		* @param array $columns 
		...
		*/
				...
				foreach($columns as $key => $value):
					switch ($key) {
						...
						case "order_product_sku":
							...
							$td_value =  isset($master_list['variations']['variation_id_sku'][$variation_id]) ? $master_list['variations']['variation_id_sku'][$variation_id] : (isset($master_list['products']['product_id_sku'][$product_id]) ? $master_list['products']['product_id_sku'][$product_id] : __("Not Set",'icwoocommerce_textdomains'));//Added 20150221
							break;
						
						case "product_sku":
							$td_value =  (isset($master_list['products']['product_id_sku'][$product_id]) ? $master_list['products']['product_id_sku'][$product_id] : __("Not Set",'icwoocommerce_textdomains'));//Added 20150221
							break;
							
						case "product_stock":
							$td_value =  (isset($master_list['products']['product_id_stock'][$product_id]) ? $master_list['products']['product_id_stock'][$product_id] : __("Not Set",'icwoocommerce_textdomains'));//Added 20150221
							break;
						...
						case "part_order_refund_amount":
							$td_value =  isset($order_item->part_order_refund_amount) ? $order_item->part_order_refund_amount : '0';
						...
						case "order_total":
							...
							$part_order_refund_amount =  isset($order_item->part_order_refund_amount) ? $order_item->part_order_refund_amount : '0';
					...
					$return_items[$rkey]->post_status 		= isset($order_item->post_status) 		? $order_item->post_status	: '';					
					
					$return_items[$rkey]->billing_country 	= isset($return_items[$rkey]->billing_country) 	? $return_items[$rkey]->billing_country		: (isset($order_item->billing_country) ? $order_item->billing_country : '');
					$return_items[$rkey]->shipping_country 	= isset($return_items[$rkey]->shipping_country) ? $return_items[$rkey]->shipping_country 	: (isset($order_item->shipping_country) ? $order_item->shipping_country : '');
				endforeach;
			endforeach;
		...
		/**
		* get_items_id_list
		* This function is used for get Item ID's.
		...
		*/
		function get_items_id_list($order_items = array(),$field_key = 'order_id', $return_default = '-1' , $return_formate = 'string'){
			...
			* This function is used for Order Item Name List's.
			...
			* This function is used for Order Item Counts.
			* @param array $order_id_string 
			* @param string $order_item_type
			* @return array
			*/
			...
			/**
			* get_variation_list
			* This function is used for Get Variations.
			* @param array $order_id_string 
			...
			* This function is used for Get Variation Attributes.
			...
			*/
			...
			/**
			* get_order_item_product_details
			* This function is used to Get Order Item Product Details.
			* @param string $order_id_string 
			* @param string $product_id_string
			...
			*/
			function get_order_item_product_details($order_id_string = '', $product_id_string = '',$product_items = array("sku")){
				...
				foreach($product_items as $key => $value){
					$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta_product_{$value} ON postmeta_product_{$value}.post_id = woocommerce_order_itemmeta_products.meta_value";
				}
				$sql .= " WHERE  1*1";
			...
			/**
			* get_order_item_sku
			* This function is used to Get Order Item SKU.
			* @param array $order_id_string 
			* @param integer $order_item_id
			* @return array
			*/
			...
			* This function is used to Get Order Product SKU List.
			* @param array $order_id_string 
			* @param integer $order_item_id
			* @return array
			*/
			...
			/**
			* get_term_names_by_id
			* This function is used to Get Term Names by ID's.
			* @param array $order_id_string 
			...
			* This function is used to Get Orders Part Refund Amount.
			* @param array $order_id_string
			* @return array
			*/
			...
			* This function is used to Get Orders Refund Amount.
			* @param array $order_id_string
			* @return array
			*/
			...
			* This function is used to Get Orders Status.
			* @return array
			*/
			...
			/**
			* get_billling_state_name
			* This function is used to Get Orders Billing State Names.
			* @param string $cc
			* @param string $st
			* @return array
			*/
			...
			* This function is used to Get Category Names by Product ID's.
			* @param string $id
			* @param string $taxonomy
			...
			*/
			...
			/**
			* get_wc_countries
			* This function is used to Get WC Countries.
			*/
			//Added 20150225
			...
			* This function is used to Get WC Currency.
			*/
