...
if ( ! class_exists( 'IC_Commerce_Ultimate_Woocommerce_Report_Variation_Stock_List_report' ) ) {
	//require_once('ic_commerce_ultimate_report_functions.php');
	
	/*
	 * Class Name IC_Commerce_Ultimate_Woocommerce_Report_Variation_Stock_List_report
	 *
	 * Class is used for returning variation stock list report 
	 *	 
	*/
	class IC_Commerce_Ultimate_Woocommerce_Report_Variation_Stock_List_report extends IC_Commerce_Ultimate_Woocommerce_Report_Functions{
		...
		*/
					...
					<div class="content">
						<div class="search_report_form">
							...
							<form action="" name="search_order_report" id="search_order_report" method="post">
                               ...
                               <?php do_action('ic_commerce_variation_stock_list_page_searrch_form_bottom',$this,'variation_stock_list');?>
                                ...
                                <div class="form-group">
                                    <div class="FormRow">
                                        ...
                                        <input type="hidden" name="report_name"		id="report_name"		value="<?php echo $this->get_request('report_name','variation_stock_list',true);?>" />
                                        ...
                                        <input name="SearchOrder" id="SearchOrder" class="onformprocess searchbtn" value="<?php _e("Search Product",'icwoocommerce_textdomains');?>" type="submit">  &nbsp; &nbsp; &nbsp; <span class="ajax_progress"></span></span>
		...
		/*
			* Function Name ic_commerce_custom_report_normal_query
			*
			* Returns all Variable products Stock list
			*
			* @param string $type
			*
		...
		*/
		...
		function ic_commerce_custom_report_normal_query($type = 'total_row'){
					...
					if(!$this->normal_sql_query){
						...
						LEFT JOIN {$wpdb->posts} as products ON products.ID = posts.post_parent";
						
						if($product_subtype=="virtual") 						$sql .= " LEFT JOIN  {$wpdb->postmeta} as virtual 			ON virtual.post_id			=posts.ID";
						
						if($product_subtype=="downloadable") 					$sql .= " LEFT JOIN  {$wpdb->postmeta} as downloadable		ON downloadable.post_id		=posts.ID";
						
						if($sku_number){
							$sql .= " LEFT JOIN  {$wpdb->postmeta} as product_sku 				ON product_sku.post_id					=	posts.post_parent";
							$sql .= " LEFT JOIN  {$wpdb->postmeta} as variation_sku 				ON variation_sku.post_id				=	posts.ID";
						}else{
						...
						if($product_stock || $txtMinStock || $txtMaxStock || strlen($product_stock) >0 || $zero_stock == "yes" || $sort_by == "variation_stock") 		$sql .= " LEFT JOIN  {$wpdb->postmeta} as stock 				ON stock.post_id			=posts.ID";
						...
						//if($zero_sold=="yes") 						$sql .= " LEFT JOIN  {$wpdb->postmeta} as total_sales 			ON total_sales.post_id			=posts.ID";//New Change ID 20150309				
						if($stock_status and $stock_status != '-1') $sql .= " LEFT JOIN  {$wpdb->postmeta} as stock_status 			ON stock_status.post_id			=posts.ID";//New Change ID 20150309
						if($manage_stock and $manage_stock != '-1') $sql .= " LEFT JOIN  {$wpdb->postmeta} as manage_stock 			ON manage_stock.post_id			=posts.ID";//New Change ID 20150309				
						
						if($variation_attributes != "-1" and strlen($variation_attributes)>2)
							$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta_variation ON postmeta_variation.post_id = posts.ID";
						
						$sql = apply_filters('ic_commerce_variation_stock_list_page_join_query',$sql,$request,$type,'',array());
						...
						if($category_id and $category_id != "-1") 	$sql .= " AND terms.term_id IN ({$category_id})";
						...
						$sql = apply_filters('ic_commerce_variation_stock_list_page_where_query',$sql,$request,$type,'',array());
								...
								<tr class="<?php echo $alternate."row_".$key;?>">
									<?php 
										...
										foreach($columns as $key => $value):
											...
											switch($key):
												case "product_name":
													$value = " <a href=\"{$admin_url}{$order_item->product_id}\" target=\"_blank\">{$order_item->product_name}</a>";
													break;
												case "Variable":
												case "product_id":
												case "variation_id":
												...
												case "stock":
												case "variation_sold":
													$value = isset($order_item->$key) ? trim($order_item->$key) : __("Not Set",'icwoocommerce_textdomains');
													$value = ($value == '') ? 0 : ($value + 0);
													$value = ($value != __("Not Set",'icwoocommerce_textdomains')) ? ($value + 0) : $value;
													$class 	= 'amount';
													break;
												case "product_date":
												case "modified_date":
		...
		function get_variation_product_sku($post_type = 'product'){
			...
			$sql = "SELECT postmeta_sku.meta_value AS id, postmeta_sku.meta_value AS label FROM `{$wpdb->posts}` AS posts";
			...
			$sql .= " LEFT JOIN {$wpdb->postmeta} AS postmeta_sku ON postmeta_sku.post_id = posts.ID";
		...
		function get_all_variation($variation = "all"){
			...
			if(count($this->all_variation) == 0){
				...
				$sql="SELECT  REPLACE(REPLACE(meta_key, 'attribute_', ''),'pa_','') as attributes  FROM {$wpdb->postmeta} WHERE meta_key like 'attribute_%'";
				if($variation_attributes != "-1" and strlen($variation_attributes)>2){
		...
		function get_product_variation($product_id){			
			...
			$sql = "SELECT meta_key, REPLACE(REPLACE(meta_key, 'attribute_', ''),'pa_','') AS attributes, meta_value 
					FROM  {$wpdb->postmeta} as postmeta WHERE post_id ='".$product_id."'
				...
				<table style="width:100%">
					<tr>
                    	<td valign="middle" class="grid_bottom_total">
                        	<?php 
								$output = sprintf(__('Results: <strong>%1$s/%2$s</strong>', 'icwoocommerce_textdomains'), $TotalOrderCount,$total_pages);
								echo $output;
							?>
						</td>
						<td>					
		...
		function ic_commerce_custom_report_page_export_csv(){
			...
			$filename 				= $export_filename = $export_file_name."-".$today.".".$export_file_format;	
			
			do_action("ic_commerce_export_csv_excel_format",$export_filename,$export_rows,$columns,$export_file_format);
				...
				foreach ( $rows as $rkey => $rvalue ):
					...
					foreach($columns as $key => $value):
						switch ($key) {
							...
							case "stock":
							case "variation_sold":
								$value = isset($order_item->$key) ? $order_item->$key : __("Not Set",'icwoocommerce_textdomains');
								$value = ($value != __("Not Set",'icwoocommerce_textdomains')) ? ($value + 0) : $value;
								break;
							case "product_date":
							case "modified_date":								
		...
		function grid_columns(){
			...
			,"sold_amount"		=>	__("Sold Amount", 		'icwoocommerce_textdomains')
			,"sold_quantity"		=>	__("Sold Qty.", 		'icwoocommerce_textdomains'));
			...
			unset($columns['day-1-meal-1']);
			unset($columns['day-1-meal-2']);
			
			unset($columns['day-2-meal-1']);
			unset($columns['day-2-meal-2']);
			unset($columns['pages']);
		...
		function get_sold_product_details($show_variation = 'variable', $item_id_string = "", $id_only = false){
			...
			if(!$this->variation_query){
				...
				$sql .= " LEFT JOIN  {$wpdb->posts} as shop_order ON shop_order.id=woocommerce_order_items.order_id";//For shop_order
