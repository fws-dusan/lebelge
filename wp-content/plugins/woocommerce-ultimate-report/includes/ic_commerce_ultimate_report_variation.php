...
if ( ! class_exists( 'IC_Commerce_Ultimate_Woocommerce_Report_Variation' ) ) {
	//require_once('ic_commerce_ultimate_report_functions.php');
	class IC_Commerce_Ultimate_Woocommerce_Report_Variation extends IC_Commerce_Ultimate_Woocommerce_Report_Functions{
		
		/*Declare per page for pagination*/
		public $per_page = 0;	
		
		/*Declare per page default for pagination*/
		public $per_page_default = 10;
		
		/*Get server request*/
		public $request_data =	array();
		
		/*Define constant variablr*/
		...
		*/
		public function __construct($constants) {
			...
			add_filter('ic_commerce_variation_page_columns', array($this, 'ic_commerce_variation_page_columns'));
		}
		
		function ic_commerce_variation_page_columns($columns){
			$new_columns = array();
			foreach($columns as $key => $column){
				if($key == 'quantity'){
					$new_columns['weight'] = __("Weight", 		'icwoocommerce_textdomains');
				}
		...
		*/
		function init(){
                                ...
                                <div class="search_report_form">
                                    ...
                                    <form action="" name="Report" id="search_order_report" method="post">
                                        <div class="form-table">
                                            ...
                                            <?php  do_action("ic_commerce_variation_page_below_date_fields", $report_name, $this );?>                                            
                ...
                <?php
                    ...
                    <style type="text/css">
						...
						th.weight, td.weight{ text-align:right;}
		...
		/**
		* ic_commerce_report_ajax_request
		* Get ajax request 
		* @param string $type
		...
		*/
			...
			if(!$this->variation_query){
				...
				if($show_variation == 'variable'){
					$sql .= " 
							...
							";
					if(($sort_by == "sku") || ($variation_sku and $variation_sku != '-1') || $variation_sku != '-1')
						$sql .= "	LEFT JOIN  {$wpdb->postmeta} as postmeta_sku 		ON postmeta_sku.post_id		= woocommerce_order_itemmeta8.meta_value";
				...
				if(($sort_by == "sku") || ($product_sku and $product_sku != '-1'))
					$sql .= "	LEFT JOIN  {$wpdb->postmeta}		 as postmeta_product_sku 		ON postmeta_product_sku.post_id 			= woocommerce_order_itemmeta_product_id.meta_value	";				
				...
				if($paid_customer  && $paid_customer != '-1' and $paid_customer != "'-1'"){
					$sql .= " 
						LEFT JOIN  {$wpdb->postmeta} 			as postmeta_billing_email				ON postmeta_billing_email.post_id=woocommerce_order_items.order_id";
				}
				
				if($billing_postcode and $billing_postcode != '-1'){
					$sql .= " LEFT JOIN  {$wpdb->postmeta} as postmeta_billing_postcode ON postmeta_billing_postcode.post_id	=	woocommerce_order_items.order_id";
				}
				...
				if($product_status != NULL  && $product_status != '-1'){
					$sql .= " LEFT JOIN {$wpdb->posts} AS products ON products.ID = woocommerce_order_itemmeta_product_id.meta_value";
				}
			...
			if($type == 'limit_row' || $type == 'all_row' or $type == 'all_row_total'){
				if(count($order_items)>0){
					...
					foreach ( $order_items as $key => $order_item ) {
						...
						$stock = isset($order_items[$key]->stock) ? $order_items[$key]->stock : '';
						
						$sku = isset($order_items[$key]->sku) ? $order_items[$key]->sku : '';
						
						if(!empty($stock)){
							$stock = $stock + 0;
						}
						
						$order_items[$key]->variation_stock = $stock;
						$order_items[$key]->product_stock = $stock;
						
						$order_items[$key]->variation_sku = $sku;
						$order_items[$key]->product_sku = $sku;
					}
				}
			}
			if($type == 'limit_row'){
		...
		/**
		* ic_commerce_custom_all_product_column_variation
		* Get all product variation columns
		* @param string $type
		* @return void 
		*/		
		...
		/**
		* get_attributes
		* Get all product variation attribute
		...
		*/
		...
		/**
		* attribute_label
		* Get all product variation attribute lable
		* @param string $name
		* @return void 
		*/
		...
		/**
		* get_variation_column_separated
		* Get all product variation attribute lable
		...
		function get_variation_comma_separated($order_item_id = 0, $variation_attributes = NULL, $all_attributes = NULL){
			global $wpdb;
				...
				foreach($order_items as $key => $value){
					if(strlen($value->variation)>0){
						
						$var = $value->attribute;
						$var = str_replace("attribute_pa_","",$var);
						$var = str_replace("attribute_","",$var);
						
						$variation[$var] = $value->variation;
					...
					switch ($key) {
							...
							*/
							...
							case "variation_stock":													
								...
								$export_rows[$i][$key] = (strlen($v)>0) ? $v : (isset($order_item->stock) ? $this->get_stock($order_item->stock) : __("Not Set",'icwoocommerce_textdomains'));
								break;
							case "product_stock":													
								$export_rows[$i][$key] = isset($order_item->stock) ? $this->get_stock($order_item->stock) : __("Not Set",'icwoocommerce_textdomains');
								break;
							case "product_sku":													
								$export_rows[$i][$key] = isset($order_item->sku) ? $this->get_stock($order_item->sku) : __("Not Set",'icwoocommerce_textdomains');
								break;
							case "variation_sku":													
								$v = get_post_meta($order_item->variation_id,"_sku",true);
								$export_rows[$i][$key] = (strlen($v)>0) ? $v : (isset($order_item->sku) ? $this->get_stock($order_item->sku) : __("Not Set",'icwoocommerce_textdomains'));
								break;
							/*
							case "color":
							case "size":
		...
		*/
		function ic_commerce_custom_report_page_export_pdf($export_file_format = "pdf"){
			...
			$output = $this->get_export_pdf_content($export_rows,$columns,$summary);			
		...
		*/
		...
		/**
		* get_all_request
		* get all srver side request and set
		*
		*
		...
		* Handle multiple selection request
		* @param string $string
		...
		*/
		...
		/**
		* get_product_variation_columns
		* 
		* Get product variation columns
		*
		*
		* @return array $grid_column 
		*/
