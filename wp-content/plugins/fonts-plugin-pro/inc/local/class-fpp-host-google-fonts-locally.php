 ...
 */
class FPP_Host_Google_Fonts_Locally {
	...
	public function get_remote_url_contents() {
		 ...
		 *
		 * The default user agent doesn't support WOFF2 or Unicode ranges.
		 */
		$user_agent = 'Mozilla/5.0 (compatible; MSIE 10.0; Macintosh; Intel Mac OS X 10_7_3; Trident/6.0)';

		// Switch to a user-agent supporting woff2 (more optimized).
	 ...
	 * @return void
	 */
	public function set_font_format( $format = 'woff' ) {
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = apply_filters( 'fpp_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'fpp_get_local_fonts_base_url', content_url() );
		}
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'fpp_get_local_fonts_subfolder_name', 'fontsplugin' );
		}
 ...
 */
function fpp_get_webfont_url( $url, $format = 'woff2' ) {
	$font = new FPP_Host_Google_Fonts_Locally( $url );
	$font->set_font_format( $format );
	return $font->get_url();
}

/**
 * Replace the external URL with a local resource.
 */
function fpp_enqueue() {
	...
	if ( $fonts->has_google_fonts() ) {
		$url = $fonts->build_url();
		wp_enqueue_style( 'olympus-google-fonts', fpp_get_webfont_url( $url, fpp_get_font_format() ), array(), OGF_VERSION );
	}
}
add_action( 'wp_enqueue_scripts', 'fpp_enqueue' );

/**
 * Return the user selected font format (WOFF or WOFF2).
 */
function fpp_get_font_format() {
	if ( ! get_theme_mod( 'fpp_use_woff2', false ) ) {
		$font_format = 'woff';
	} else {
		$font_format = 'woff2';
	}
...
/**
 * Change the subfolder name where font files are stored.
 */
function fpp_reset_cache() {
	if ( ! empty( $_GET['action'] ) && 'fpp-reset-cache' === $_GET['action'] ) {
		$loader = new FPP_Host_Google_Fonts_Locally();
		$loader->delete_fonts_folder();
	}
}
add_action( 'init', 'fpp_reset_cache' );
