...
if ( !class_exists('WooMail_Puc_v4p4_StateStore', false) ):

	class WooMail_Puc_v4p4_StateStore {
		/**
		...
		/**
		 * @var WooMail_Puc_v4p4_Update|null Cached update.
		 */
		...
		/**
		 * @return null|WooMail_Puc_v4p4_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(WooMail_Puc_v4p4_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( WooMail_Puc_v4p4_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(WooMail_Puc_v4p4_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = WooMail_Puc_v4p4_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
