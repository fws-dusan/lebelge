...
if ( ! class_exists( 'WCAP_DB_Layer' ) ) {
	...
	class WCAP_DB_Layer {
		 ...
		 */
		public static function insert_cart_history( $user_id, $cart_info, $abandoned_time, $ignored, $recovered, $unsubscribe = '0', $user_type, $language, $session_id, $ip_address, $email_reminder_status, $wcap_trash ) {
			...
			if ( defined ( 'WOOCOMMERCE_MULTICURRENCY_VERSION' ) ) {
				$cart_info = self::add_wc_multicurrency( $cart_info );
			}
			...
			$wpdb->insert( 
				...
				array( 
					...
					'email_reminder_status' => $email_reminder_status,
