...
use FlexibleShippingImportExportVendor\PhpConsole\Handler;
 ...
 */
class PHPConsoleHandler extends \FlexibleShippingImportExportVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FlexibleShippingImportExportVendor\PhpConsole\Connector $connector = null, $level = \FlexibleShippingImportExportVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
    ...
    private function initConnector(\FlexibleShippingImportExportVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FlexibleShippingImportExportVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FlexibleShippingImportExportVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FlexibleShippingImportExportVendor\PhpConsole\Helper::isRegistered()) {
            \FlexibleShippingImportExportVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FlexibleShippingImportExportVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FlexibleShippingImportExportVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FlexibleShippingImportExportVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FlexibleShippingImportExportVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
