...
function bom_match(string $str) : string
{
    static $list;
    if (null === $list) {
        $list = (new \ReflectionClass(\FlexibleShippingImportExportVendor\League\Csv\ByteSequence::class))->getConstants();
        \rsort($list);
    }
 ...
 * @return int[]
 */
function delimiter_detect(\FlexibleShippingImportExportVendor\League\Csv\Reader $csv, array $delimiters, int $limit = 1) : array
{
    ...
    $stmt = (new \FlexibleShippingImportExportVendor\League\Csv\Statement())->limit($limit);
...
use FlexibleShippingImportExportVendor\League\Csv;
if (\PHP_VERSION_ID < 70100 && !\function_exists('FlexibleShippingImportExportVendor\\is_iterable')) {
    /**
     * @codeCoverageIgnore
     */
    function is_iterable($iterable)
    {
        return \FlexibleShippingImportExportVendor\League\Csv\is_iterable($iterable);
    }
}
