...
class Stream implements \SeekableIterator
{
    ...
    public static function createFromPath(string $path, string $open_mode = 'r', $context = null)
    {
        ...
        if (!($resource = @\fopen(...$args))) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\Exception(\sprintf('`%s`: failed to open stream: No such file or directory', $path));
        }
    ...
    public function appendFilter(string $filtername, int $read_write, $params = null)
    {
        ...
        throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('unable to locate filter `%s`', $filtername));
    }
    /**
     * Set CSV control.
     *
    ...
    protected function filterControl(string $delimiter, string $enclosure, string $escape, string $caller) : array
    {
        if (1 !== \strlen($delimiter)) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects delimiter to be a single character', $caller));
        }
        if (1 !== \strlen($enclosure)) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects enclosure to be a single character', $caller));
        }
        if (1 === \strlen($escape) || '' === $escape && 70400 <= \PHP_VERSION_ID) {
            return [$delimiter, $enclosure, $escape];
        }
        throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects escape to be a single character', $caller));
    }
    /**
     * Set CSV control.
     *
    ...
    {
        if (!$this->is_seekable) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\Exception('stream does not support seeking');
        }
        \rewind($this->stream);
        $this->offset = 0;
        $this->value = \false;
    ...
    {
        if ($position < 0) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\Exception(\sprintf('%s() can\'t seek stream to negative line %d', __METHOD__, $position));
        }
        $this->rewind();
