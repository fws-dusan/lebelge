...
namespace FlexibleShippingImportExportVendor\League\Csv\Polyfill;

use Generator;
use FlexibleShippingImportExportVendor\League\Csv\Stream;
use SplFileObject;
use TypeError;
...
final class EmptyEscapeParser
{
    ...
    public static function parse($document) : \Generator
    {
        ...
        self::$trim_mask = \str_replace([self::$delimiter, self::$enclosure], '', " \t\0\v");
    ...
    {
        if ($document instanceof \FlexibleShippingImportExportVendor\League\Csv\Stream || $document instanceof \SplFileObject) {
            return $document;
        }
        throw new \TypeError(\sprintf('%s::parse expects parameter 1 to be a %s or a SplFileObject object, %s given', self::class, \FlexibleShippingImportExportVendor\League\Csv\Stream::class, \is_object($document) ? \get_class($document) : \gettype($document)));
    }
    /**
