...
class Statement
{
    ...
    {
        if (0 > $offset) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects the offset to be a positive integer or 0, %s given', __METHOD__, $offset));
        }
    ...
    {
        if (-1 > $limit) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects the limit to be greater or equal to -1, %s given', __METHOD__, $limit));
        }
        if ($limit === $this->limit) {
            return $this;
        }
     ...
     */
    public function process(\FlexibleShippingImportExportVendor\League\Csv\Reader $csv, array $header = []) : \FlexibleShippingImportExportVendor\League\Csv\ResultSet
    {
        if ([] === $header) {
        ...
        return new \FlexibleShippingImportExportVendor\League\Csv\ResultSet(new \LimitIterator($iterator, $this->offset, $this->limit), $header);
    }
    /**
