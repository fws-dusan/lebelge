...
class ResultSet implements \Countable, \IteratorAggregate, \JsonSerializable
{
    ...
    {
        if ($nth_record < 0) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf('%s() expects the submitted offset to be a positive integer or 0, %s given', __METHOD__, $nth_record));
        }
    ...
    public function fetchColumn($index = 0) : \Generator
    {
        ...
        $iterator = new \FlexibleShippingImportExportVendor\League\Csv\MapIterator(new \CallbackFilterIterator($this->records, $filter), $select);
    ...
    protected function getColumnIndexByValue(string $value, string $error_message) : string
    {
        ...
        throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf($error_message, $value));
    }
    /**
    ...
    {
        if ($index < 0) {
            throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument($error_message);
        }
        if ([] === $this->header) {
            return $index;
        }
        ...
        throw new \FlexibleShippingImportExportVendor\League\Csv\InvalidArgument(\sprintf($error_message, $index));
    }
    /**
