 ...
 */
class LicensePage implements \FlexibleShippingImportExportVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FlexibleShippingImportExportVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FlexibleShippingImportExportVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
