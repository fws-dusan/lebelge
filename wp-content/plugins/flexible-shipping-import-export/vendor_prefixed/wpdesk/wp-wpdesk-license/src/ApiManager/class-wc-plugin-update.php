    ...
    class WPDesk_Update_API_Check
    {
        ...
        public function plugin_information($args)
        {
            ...
            if (\is_wp_error($request) || \wp_remote_retrieve_response_code($request) != 200) {
                if (\class_exists('FlexibleShippingImportExportVendor\\WPDesk_Logger_Factory')) {
                    if (\is_wp_error($request)) {
                        \FlexibleShippingImportExportVendor\WPDesk_Logger_Factory::log_wp_error($request, \debug_backtrace());
                    } else {
                        \FlexibleShippingImportExportVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is invalid. Response: ' . \json_encode($request), \FlexibleShippingImportExportVendor\WPDesk_Logger::ERROR, \debug_backtrace());
                    }
                }
                return \false;
            }
            ...
            if (\is_object($response)) {
                ...
                if (\class_exists('FlexibleShippingImportExportVendor\\WPDesk_Logger_Factory')) {
                    \FlexibleShippingImportExportVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is not an object', \FlexibleShippingImportExportVendor\WPDesk_Logger::DEBUG, \debug_backtrace());
                } else {
