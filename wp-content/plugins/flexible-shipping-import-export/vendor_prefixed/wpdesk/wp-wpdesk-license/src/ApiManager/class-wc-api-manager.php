...
if (!\class_exists('FlexibleShippingImportExportVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
        ...
        private function add_not_possible_update_message()
        {
            \add_action('in_plugin_update_message-' . $this->plugin_name, function (array $plugin_data, \stdClass $response) {
                if (isset($response, $response->package) && empty($response->package)) {
                    echo \sprintf(\__(" <a target='_blank' href='%s'>Enter a valid subscription key for automatic updates.</a>", 'flexible-shipping-import-export'), \admin_url('admin.php?page=wpdesk-licenses'));
                }
        ...
        public function create_instance_id()
        {
            ...
            $password_management = new \FlexibleShippingImportExportVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \FlexibleShippingImportExportVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \FlexibleShippingImportExportVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function inactive_notice()
        {
            ...
            if (\apply_filters('wpdesk_show_plugin_activation_notice', \true)) {
                (new \FlexibleShippingImportExportVendor\WPDesk\Notice\AjaxHandler())->hooks();
                new \FlexibleShippingImportExportVendor\WPDesk\Notice\PermanentDismissibleNotice(\sprintf(\__('The %s%s%s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %sClick here%s to activate the API key and the plugin.', $this->text_domain), '<strong>', $this->software_product_id, '</strong>', '<a href="' . \esc_url(\admin_url('admin.php?page=wpdesk-licenses')) . '">', '</a>'), "notice-{$this->software_product_id}", \FlexibleShippingImportExportVendor\WPDesk\Notice\PermanentDismissibleNotice::NOTICE_TYPE_WARNING);
            }
        }
        /**
        ...
        private function display_changelog($plugin_version, $changelog)
        {
            ...
            if (\iterator_count($changes) > 0) {
                $changelog = new \FlexibleShippingImportExportVendor\WPDesk\License\Changelog\Formatter($changes);
