 ...
 */
class SimplePaidStrategy implements \FlexibleShippingImportExportVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    public function run_init(\FlexibleShippingImportExportVendor\WPDesk_Plugin_Info $plugin_info)
    {
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \FlexibleShippingImportExportVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \FlexibleShippingImportExportVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \FlexibleShippingImportExportVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \FlexibleShippingImportExportVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \FlexibleShippingImportExportVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
