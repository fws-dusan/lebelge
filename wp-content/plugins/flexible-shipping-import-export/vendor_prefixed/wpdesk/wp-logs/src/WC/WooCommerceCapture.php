...
class WooCommerceCapture
{
    ...
    public function __construct(\FlexibleShippingImportExportVendor\Monolog\Logger $monolog)
    {
    ...
    private function prepareCaptureHookCallable()
    {
        ...
        if ($this->captureHookFunction === null) {
            $this->captureHookFunction = function () use($monolog) {
                return new \FlexibleShippingImportExportVendor\WPDesk\Logger\WC\WooCommerceMonologPlugin($monolog, $this->originalWCLogger);
            };
            $this->monolog->pushHandler(new \FlexibleShippingImportExportVendor\WPDesk\Logger\WC\WooCommerceHandler($this->originalWCLogger));
        }
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            if ($this->isCaptured) {
                throw new \FlexibleShippingImportExportVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured('Try to free wc logger first.');
            }
