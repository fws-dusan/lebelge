 ...
 */
class WPDeskLoggerFactory extends \FlexibleShippingImportExportVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FlexibleShippingImportExportVendor\Monolog\Registry::hasLogger($name)) {
            \FlexibleShippingImportExportVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FlexibleShippingImportExportVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FlexibleShippingImportExportVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FlexibleShippingImportExportVendor\Monolog\Logger($name);
        }
        if (\FlexibleShippingImportExportVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FlexibleShippingImportExportVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FlexibleShippingImportExportVendor\Monolog\Logger $logger)
    {
        ...
        $wcIntegration = new \FlexibleShippingImportExportVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FlexibleShippingImportExportVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FlexibleShippingImportExportVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FlexibleShippingImportExportVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FlexibleShippingImportExportVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FlexibleShippingImportExportVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FlexibleShippingImportExportVendor\Monolog\Registry::hasLogger($name);
    }
}
