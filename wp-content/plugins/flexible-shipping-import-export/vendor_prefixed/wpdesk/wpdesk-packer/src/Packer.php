...
namespace FlexibleShippingImportExportVendor\WPDesk\Packer;

use FlexibleShippingImportExportVendor\WPDesk\Packer\Exception\NoItemsException;
/**
...
class Packer
{
     ...
     * @param Item $item
     */
    public function add_item(\FlexibleShippingImportExportVendor\WPDesk\Packer\Item $item)
    {
        $this->items[] = $item;
    }
    /**
     * @param Box $box
     */
    public function add_box(\FlexibleShippingImportExportVendor\WPDesk\Packer\Box $box)
    {
        $this->boxes[] = $box;
    }
    /**
    ...
    public function pack()
    {
        if (\sizeof($this->items) === 0) {
            throw new \FlexibleShippingImportExportVendor\WPDesk\Packer\Exception\NoItemsException('No items to pack!');
        }
        $this->packages = [];
    ...
    private function find_best_packed_box()
    {
        ...
        foreach ($this->boxes as $box) {
            $packages[] = new \FlexibleShippingImportExportVendor\WPDesk\Packer\PackedBox($box, $this->items);
        }
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\FlexibleShippingImportExportVendor\WPDesk\Packer\Box $a, \FlexibleShippingImportExportVendor\WPDesk\Packer\Box $b) {
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\FlexibleShippingImportExportVendor\WPDesk\Packer\Item $a, \FlexibleShippingImportExportVendor\WPDesk\Packer\Item $b) {
                if ($a->get_volume() === $b->get_volume()) {
