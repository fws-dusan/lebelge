(function( $) {
  ...
  $( document ).ready(function( ) {
    ...
    function validateFieldsInSlide( stepsElm, event, currentIndex ) {
      ...
      //  check form fields on the current slide
      var fields = getVisibleSlide( stepsElm ).find( 'input,select' );
      var valid = true;
      if (fields.length > 0) {
      ...
      $( wizardId ).dialog({
        ...
        open: function() {
          ...
          stepsElm.steps({
            ...
            {
              stepsElm.validate().settings.ignore = ":disabled";
              return stepsElm.valid();
            },
