...
use FSProVendor\WPDesk\Helper\Integration\LicenseIntegration;
use FSProVendor\WPDesk\Helper\Integration\LogsIntegration;
use FSProVendor\WPDesk\Helper\Integration\SettingsIntegration;
use FSProVendor\WPDesk\Helper\Integration\TrackerIntegration;
use FSProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use FSProVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \FSProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \FSProVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \FSProVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \FSProVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \FSProVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \FSProVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \FSProVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \FSProVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \FSProVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \FSProVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \FSProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
