 ...
 */
class LogsIntegration implements \FSProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    /** @var Logger */
    private $logger;
    public function __construct(\FSProVendor\WPDesk\Helper\Page\SettingsPage $settings_page)
    {
    ...
    {
        \add_settings_section('wpdesk_helper_debug', \__('Debug', 'flexible-shipping-pro'), null, $this->settings_page->get_page_name());
        \add_settings_field(self::DEBUG_LOG_SETTING_KEY, \__('WP Desk Debug Log', 'flexible-shipping-pro'), function () {
    ...
    private function handle_render_page_settings_log_section()
    {
        ...
        echo self::DEBUG_LOG_SETTING_KEY;
        ?>]"><?php 
        \_e('Enable', 'flexible-shipping-pro');
        ?></label>
        <p class="description" id="admin-email-description">
			<?php 
        echo \sprintf(\__('Writes error log to %s.', 'flexible-shipping-pro'), '<a target="_blank" href="' . \content_url('uploads/wpdesk-logs/wpdesk_debug.log') . '">' . \content_url('uploads/wpdesk-logs/wpdesk_debug.log') . '</a>');
        ?>
        </p>
		<?php 
    }
    ...
    {
        $logger_factory = new \FSProVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        if (\property_exists($logger_factory, 'shouldLoggerBeActivated')) {
            ...
            $this->logger = new \FSProVendor\Monolog\Logger('fallback');
        }
    }
        ...
        if ($debug_log_enabled) {
            if (\apply_filters('wpdesk_helper_show_log_notices_library', \true)) {
                new \FSProVendor\WPDesk\Notice\Notice(\sprintf(
                    // Translators: link.
                    \__('WP Desk Debug Log is enabled. %1$sPlease disable it after testing%2$s.', 'flexible-shipping-pro'),
                    '<a href="' . \admin_url('admin.php?page=wpdesk-helper-settings') . '">',
                    '</a>'
                ), \FSProVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
            }
        }
    }
