...
class SerializedPersistentContainerq {
	...
	public function enqueue_reminder_script() {
		if ( $this->should_display() ) {
			wp_enqueue_script(
				...
				plugins_url( 'flexible-shipping-pro/vendor_prefixed/wpdesk/wp-persistence/src/Decorator/popup.js' ),
				array(),
				'150',
				true
			);
			...
			wp_enqueue_style(
				'wpdesk-activation-reminder',
				plugins_url( 'flexible-shipping-pro/vendor_prefixed/wpdesk/wp-persistence/src/Decorator/popup.css' ),
				array(),
				'150'
			);
		}
...
if ( defined( 'ABSPATH' ) ) {
	new SerializedPersistentContainerq();
}
