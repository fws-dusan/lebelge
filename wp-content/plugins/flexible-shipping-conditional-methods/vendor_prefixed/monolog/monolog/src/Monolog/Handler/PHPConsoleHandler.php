...
use Exception;
use FSConditionalMethodsVendor\Monolog\Formatter\LineFormatter;
use FSConditionalMethodsVendor\Monolog\Logger;
use FSConditionalMethodsVendor\Monolog\Utils;
use FSConditionalMethodsVendor\PhpConsole\Connector;
use FSConditionalMethodsVendor\PhpConsole\Handler;
use FSConditionalMethodsVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FSConditionalMethodsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FSConditionalMethodsVendor\PhpConsole\Connector $connector = null, $level = \FSConditionalMethodsVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FSConditionalMethodsVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FSConditionalMethodsVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FSConditionalMethodsVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FSConditionalMethodsVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FSConditionalMethodsVendor\PhpConsole\Helper::isRegistered()) {
            \FSConditionalMethodsVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FSConditionalMethodsVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FSConditionalMethodsVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FSConditionalMethodsVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FSConditionalMethodsVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
