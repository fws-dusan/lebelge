 ...
 */
namespace FSConditionalMethodsVendor\Monolog;
...
use FSConditionalMethodsVendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \FSConditionalMethodsVendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\FSConditionalMethodsVendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \FSConditionalMethodsVendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \FSConditionalMethodsVendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \FSConditionalMethodsVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \FSConditionalMethodsVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
