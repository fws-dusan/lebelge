 ...
 */
namespace FSConditionalMethodsVendor\WPDesk\RepositoryRating;

use FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\Hookable;
/**
 * Can display rating notices based on watcher time time.
 */
class RatingPetitionNotice implements \FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(\FSConditionalMethodsVendor\WPDesk\RepositoryRating\TimeWatcher $method_watcher, $namespace, $plugin_name, $rate_url = '')
    {
    ...
    {
        new \FSConditionalMethodsVendor\WPDesk\Notice\PermanentDismissibleNotice($this->get_notice_content(), $notice_name, \FSConditionalMethodsVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
    }
    /**
     * Get notice content.
     *
    ...
    private function get_notice_content()
    {
        ...
        $content = \sprintf(\__('Awesome, you\'ve been using %s for more than 2 weeks. Could you please do me a BIG favor and give it a 5-star rating on WordPress? ~ Peter', 'flexible-shipping-conditional-methods'), $this->plugin_name);
        $content .= '<br/>';
    ...
    protected function action_links()
    {
        $actions[] = \sprintf(
            // phpcs:ignore
            // Translators: link.
            \__('%1$sOk, you deserved it%2$s', 'flexible-shipping-conditional-methods'),
            '<a class="fs-response-deserved" target="_blank" href="' . \esc_url($this->rate_url) . '">',
            '</a>'
        );
        $actions[] = \sprintf(
            // Translators: link.
            \__('%1$sNope, maybe later%2$s', 'flexible-shipping-conditional-methods'),
            '<a class="fs-response-close-temporary-notice notice-dismiss-link" data-source="' . self::CLOSE_TEMPORARY_NOTICE . '" href="#">',
            '</a>'
        );
        $actions[] = \sprintf(
            // Translators: link.
            \__('%1$sI already did%2$s', 'flexible-shipping-conditional-methods'),
