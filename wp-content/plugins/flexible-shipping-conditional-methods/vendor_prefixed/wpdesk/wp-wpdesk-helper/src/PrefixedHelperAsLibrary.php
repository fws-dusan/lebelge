...
namespace FSConditionalMethodsVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use FSConditionalMethodsVendor\WPDesk\Helper\Debug\LibraryDebug;
use FSConditionalMethodsVendor\WPDesk\Helper\Integration\LicenseIntegration;
use FSConditionalMethodsVendor\WPDesk\Helper\Integration\LogsIntegration;
use FSConditionalMethodsVendor\WPDesk\Helper\Integration\SettingsIntegration;
use FSConditionalMethodsVendor\WPDesk\Helper\Integration\TrackerIntegration;
use FSConditionalMethodsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use FSConditionalMethodsVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const LIBRARY_TEXT_DOMAIN = 'flexible-shipping-conditional-methods';
    ...
    {
        $helper_info = new \FSConditionalMethodsVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \FSConditionalMethodsVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \FSConditionalMethodsVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \FSConditionalMethodsVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \FSConditionalMethodsVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \FSConditionalMethodsVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \FSConditionalMethodsVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \FSConditionalMethodsVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \FSConditionalMethodsVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \FSConditionalMethodsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
