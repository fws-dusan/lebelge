...
namespace FSConditionalMethodsVendor\WPDesk\View;
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \FSConditionalMethodsVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \FSConditionalMethodsVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \FSConditionalMethodsVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \FSConditionalMethodsVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
