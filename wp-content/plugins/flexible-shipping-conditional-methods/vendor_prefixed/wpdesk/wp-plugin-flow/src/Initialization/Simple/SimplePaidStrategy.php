...
namespace FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\Simple;

use FSConditionalMethodsVendor\WPDesk\Helper\HelperRemover;
use FSConditionalMethodsVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use FSConditionalMethodsVendor\WPDesk\License\PluginRegistrator;
use FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
use FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin;
/**
 ...
 */
class SimplePaidStrategy implements \FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    /** @var SlimPlugin */
    private $plugin;
    public function __construct(\FSConditionalMethodsVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\FSConditionalMethodsVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\FSConditionalMethodsVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \FSConditionalMethodsVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \FSConditionalMethodsVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \FSConditionalMethodsVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \FSConditionalMethodsVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
