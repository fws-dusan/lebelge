...
use LogicException;
use FSConditionalMethodsVendor\Monolog\Handler\NullHandler;
use FSConditionalMethodsVendor\Monolog\Logger;
use FSConditionalMethodsVendor\Monolog\Registry;
use FSConditionalMethodsVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use FSConditionalMethodsVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FSConditionalMethodsVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FSConditionalMethodsVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FSConditionalMethodsVendor\Monolog\Registry::hasLogger($name)) {
            \FSConditionalMethodsVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FSConditionalMethodsVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FSConditionalMethodsVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FSConditionalMethodsVendor\Monolog\Logger($name);
        }
        if (\FSConditionalMethodsVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FSConditionalMethodsVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FSConditionalMethodsVendor\Monolog\Logger $logger)
    {
        if (!\defined('FSConditionalMethodsVendor\\WC_LOG_THRESHOLD')) {
            \define('FSConditionalMethodsVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FSConditionalMethodsVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FSConditionalMethodsVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FSConditionalMethodsVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FSConditionalMethodsVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FSConditionalMethodsVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FSConditionalMethodsVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FSConditionalMethodsVendor\Monolog\Registry::hasLogger($name);
    }
}
