...
namespace FSConditionalMethodsVendor\WPDesk\Logger\WC;

use FSConditionalMethodsVendor\Monolog\Logger;
use FSConditionalMethodsVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured;
/**
...
class WooCommerceCapture
{
    ...
    public function __construct(\FSConditionalMethodsVendor\Monolog\Logger $monolog)
    {
    ...
    private function prepareCaptureHookCallable()
    {
        ...
        if ($this->captureHookFunction === null) {
            $this->captureHookFunction = function () use($monolog) {
                return new \FSConditionalMethodsVendor\WPDesk\Logger\WC\WooCommerceMonologPlugin($monolog, $this->originalWCLogger);
            };
            $this->monolog->pushHandler(new \FSConditionalMethodsVendor\WPDesk\Logger\WC\WooCommerceHandler($this->originalWCLogger));
        }
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            if ($this->isCaptured) {
                throw new \FSConditionalMethodsVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured('Try to free wc logger first.');
            }
