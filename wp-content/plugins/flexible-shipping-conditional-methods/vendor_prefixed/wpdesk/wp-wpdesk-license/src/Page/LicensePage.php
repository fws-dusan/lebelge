...
use FSConditionalMethodsVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \FSConditionalMethodsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FSConditionalMethodsVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    {
        \add_submenu_page('wpdesk-helper', \__('Subscriptions', 'flexible-shipping-conditional-methods'), \__('Subscriptions', 'flexible-shipping-conditional-methods'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('FSConditionalMethodsVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FSConditionalMethodsVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
