...
use FSConditionalMethodsVendor\WPDesk\License\ServerAddressRepository;
use FSConditionalMethodsVendor\WPDesk_API_Manager_With_Update_Flag;
/**
 * Can activate plugin license.
 *
 * @package WPDesk\License\Page\License\Action
 */
class LicenseActivation implements \FSConditionalMethodsVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \FSConditionalMethodsVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        \add_settings_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-shipping-conditional-methods'), 'error');
    }
    /**
    ...
    {
        \add_settings_error('activate_text', 'activate_msg', \__('Plugin activated. ', 'flexible-shipping-conditional-methods') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \FSConditionalMethodsVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
