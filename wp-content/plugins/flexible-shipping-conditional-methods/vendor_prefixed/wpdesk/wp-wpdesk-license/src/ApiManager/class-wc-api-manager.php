...
use FSConditionalMethodsVendor\WPDesk\Notice\AjaxHandler;
use FSConditionalMethodsVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('FSConditionalMethodsVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
         ...
         */
        public $text_domain = 'flexible-shipping-conditional-methods';
        /**
         * @var string
         */
        public $plugin_url;
        ...
        private function add_not_possible_update_message()
        {
            \add_action('in_plugin_update_message-' . $this->plugin_name, function (array $plugin_data, \stdClass $response) {
                if (isset($response, $response->package) && empty($response->package)) {
                    echo \sprintf(\__(" <a target='_blank' href='%s'>Enter a valid subscription key for automatic updates.</a>", 'flexible-shipping-conditional-methods'), \admin_url('admin.php?page=wpdesk-licenses'));
                }
        ...
        public function create_instance_id()
        {
            ...
            $password_management = new \FSConditionalMethodsVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \FSConditionalMethodsVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \FSConditionalMethodsVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function inactive_notice()
        {
            ...
            if (\apply_filters('wpdesk_show_plugin_activation_notice', \true)) {
                (new \FSConditionalMethodsVendor\WPDesk\Notice\AjaxHandler())->hooks();
                new \FSConditionalMethodsVendor\WPDesk\Notice\PermanentDismissibleNotice(\sprintf(\__('The %s%s%s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %sClick here%s to activate the API key and the plugin.', $this->text_domain), '<strong>', $this->software_product_id, '</strong>', '<a href="' . \esc_url(\admin_url('admin.php?page=wpdesk-licenses')) . '">', '</a>'), "notice-{$this->software_product_id}", \FSConditionalMethodsVendor\WPDesk\Notice\PermanentDismissibleNotice::NOTICE_TYPE_WARNING);
            }
        }
        /**
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('FSConditionalMethodsVendor\\WP_HTTP_BLOCK_EXTERNAL') && \FSConditionalMethodsVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('FSConditionalMethodsVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\FSConditionalMethodsVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
        ...
        {
            $parser = new \FSConditionalMethodsVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \FSConditionalMethodsVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \FSConditionalMethodsVendor\WPDesk\License\Changelog\Formatter($changes);
