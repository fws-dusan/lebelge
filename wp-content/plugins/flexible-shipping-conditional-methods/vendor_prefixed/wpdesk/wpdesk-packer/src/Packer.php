...
use FSConditionalMethodsVendor\WPDesk\Packer\Exception\NoItemsException;
/**
...
class Packer
{
     ...
     * @param Item $item
     */
    public function add_item(\FSConditionalMethodsVendor\WPDesk\Packer\Item $item)
    {
        $this->items[] = $item;
    }
    /**
     * @param Box $box
     */
    public function add_box(\FSConditionalMethodsVendor\WPDesk\Packer\Box $box)
    {
        $this->boxes[] = $box;
    }
    /**
    ...
    public function pack()
    {
        if (\sizeof($this->items) === 0) {
            throw new \FSConditionalMethodsVendor\WPDesk\Packer\Exception\NoItemsException('No items to pack!');
        }
        $this->packages = [];
    ...
    private function find_best_packed_box()
    {
        ...
        foreach ($this->boxes as $box) {
            $packages[] = new \FSConditionalMethodsVendor\WPDesk\Packer\PackedBox($box, $this->items);
        }
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\FSConditionalMethodsVendor\WPDesk\Packer\Box $a, \FSConditionalMethodsVendor\WPDesk\Packer\Box $b) {
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\FSConditionalMethodsVendor\WPDesk\Packer\Item $a, \FSConditionalMethodsVendor\WPDesk\Packer\Item $b) {
                if ($a->get_volume() === $b->get_volume()) {
