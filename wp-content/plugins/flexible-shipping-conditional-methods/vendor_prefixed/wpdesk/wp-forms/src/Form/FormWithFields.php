...
namespace FSConditionalMethodsVendor\WPDesk\Forms\Form;

use Psr\Container\ContainerInterface;
use FSConditionalMethodsVendor\WPDesk\Forms\ContainerForm;
use FSConditionalMethodsVendor\WPDesk\Forms\Field;
use FSConditionalMethodsVendor\WPDesk\Forms\FieldProvider;
use FSConditionalMethodsVendor\WPDesk\Forms\Form;
use FSConditionalMethodsVendor\WPDesk\Persistence\Adapter\ArrayContainer;
use FSConditionalMethodsVendor\WPDesk\Persistence\ElementNotExistsException;
...
class FormWithFields implements \FSConditionalMethodsVendor\WPDesk\Forms\Form, \FSConditionalMethodsVendor\WPDesk\Forms\ContainerForm, \FSConditionalMethodsVendor\WPDesk\Forms\FieldProvider
{
     ...
     * @inheritDoc
     */
    public function add_field(\FSConditionalMethodsVendor\WPDesk\Forms\Field $field)
    {
        $this->fields[] = $field;
    }
    /**
    ...
    {
        if (\is_array($data)) {
            $data = new \FSConditionalMethodsVendor\WPDesk\Persistence\Adapter\ArrayContainer($data);
        }
        foreach ($this->fields as $field) {
            $data_key = $field->get_name();
            if ($data->has($data_key)) {
                ...
                } catch (\FSConditionalMethodsVendor\WPDesk\Persistence\ElementNotExistsException $e) {
     ...
     * @return string
     */
    public function render_fields(\FSConditionalMethodsVendor\WPDesk\View\Renderer\Renderer $renderer)
    {
        $content = '';
     ...
     * @inheritDoc
     */
    public function render_form(\FSConditionalMethodsVendor\WPDesk\View\Renderer\Renderer $renderer)
    {
