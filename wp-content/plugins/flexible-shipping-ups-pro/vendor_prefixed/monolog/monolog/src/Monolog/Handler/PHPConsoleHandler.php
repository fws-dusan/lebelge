...
use Exception;
use UpsProVendor\Monolog\Formatter\LineFormatter;
use UpsProVendor\Monolog\Logger;
use UpsProVendor\Monolog\Utils;
use UpsProVendor\PhpConsole\Connector;
use UpsProVendor\PhpConsole\Handler;
use UpsProVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \UpsProVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \UpsProVendor\PhpConsole\Connector $connector = null, $level = \UpsProVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('UpsProVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\UpsProVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \UpsProVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \UpsProVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\UpsProVendor\PhpConsole\Helper::isRegistered()) {
            \UpsProVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \UpsProVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \UpsProVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \UpsProVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \UpsProVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
