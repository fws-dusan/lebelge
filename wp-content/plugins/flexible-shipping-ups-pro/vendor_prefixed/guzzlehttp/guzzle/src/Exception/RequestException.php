 ...
 */
class RequestException extends \UpsProVendor\GuzzleHttp\Exception\TransferException
{
    ...
    public function __construct($message, \Psr\Http\Message\RequestInterface $request, \Psr\Http\Message\ResponseInterface $response = null, \Exception $previous = null, array $handlerContext = [])
    {
        ...
        $code = $response && !$response instanceof \UpsProVendor\GuzzleHttp\Promise\PromiseInterface ? $response->getStatusCode() : 0;
    ...
    {
        return $e instanceof \UpsProVendor\GuzzleHttp\Exception\RequestException ? $e : new \UpsProVendor\GuzzleHttp\Exception\RequestException($e->getMessage(), $request, null, $e);
    }
    /**
    ...
    public static function create(\Psr\Http\Message\RequestInterface $request, \Psr\Http\Message\ResponseInterface $response = null, \Exception $previous = null, array $ctx = [])
    {
        ...
        if ($level === 4) {
            $label = 'Client error';
            $className = \UpsProVendor\GuzzleHttp\Exception\ClientException::class;
        } elseif ($level === 5) {
            $label = 'Server error';
            $className = \UpsProVendor\GuzzleHttp\Exception\ServerException::class;
        } else {
    ...
    {
        return \UpsProVendor\GuzzleHttp\Psr7\get_message_body_summary($response);
    }
    /**
     ...
     * @return UriInterface
     */
    private static function obfuscateUri(\Psr\Http\Message\UriInterface $uri)
    {
