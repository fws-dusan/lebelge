...
use UpsProVendor\GuzzleHttp\Promise\PromiseInterface;
...
class StreamHandler
{
    ...
    public function __invoke(\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        $startTime = isset($options['on_stats']) ? \UpsProVendor\GuzzleHttp\_current_time() : null;
        try {
            ...
            if (\strpos($message, 'getaddrinfo') || \strpos($message, 'Connection refused') || \strpos($message, "couldn't connect to host") || \strpos($message, "connection attempt failed")) {
                $e = new \UpsProVendor\GuzzleHttp\Exception\ConnectException($e->getMessage(), $request, $e);
            }
            $e = \UpsProVendor\GuzzleHttp\Exception\RequestException::wrapException($request, $e);
            $this->invokeStats($options, $request, $startTime, null, $e);
            return \UpsProVendor\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    private function invokeStats(array $options, \Psr\Http\Message\RequestInterface $request, $startTime, \Psr\Http\Message\ResponseInterface $response = null, $error = null)
    {
        if (isset($options['on_stats'])) {
            $stats = new \UpsProVendor\GuzzleHttp\TransferStats($request, $response, \UpsProVendor\GuzzleHttp\_current_time() - $startTime, $error, []);
    ...
    private function createResponse(\Psr\Http\Message\RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \UpsProVendor\GuzzleHttp\headers_from_lines($hdrs);
        list($stream, $headers) = $this->checkDecode($options, $headers, $stream);
        $stream = \UpsProVendor\GuzzleHttp\Psr7\stream_for($stream);
        $sink = $stream;
        if (\strcasecmp('HEAD', $request->getMethod())) {
            $sink = $this->createSink($stream, $options);
        }
        $response = new \UpsProVendor\GuzzleHttp\Psr7\Response($status, $headers, $sink, $ver, $reason);
        if (isset($options['on_headers'])) {
            try {
                $options['on_headers']($response);
            } catch (\Exception $e) {
                $msg = 'An error was encountered during the on_headers event';
                $ex = new \UpsProVendor\GuzzleHttp\Exception\RequestException($msg, $request, $response, $e);
                return \UpsProVendor\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
        ...
        return new \UpsProVendor\GuzzleHttp\Promise\FulfilledPromise($response);
    }
    private function createSink(\Psr\Http\Message\StreamInterface $stream, array $options)
    {
        ...
        return \is_string($sink) ? new \UpsProVendor\GuzzleHttp\Psr7\LazyOpenStream($sink, 'w+') : \UpsProVendor\GuzzleHttp\Psr7\stream_for($sink);
    }
    private function checkDecode(array $options, array $headers, $stream)
    {
        // Automatically decode responses when instructed.
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \UpsProVendor\GuzzleHttp\normalize_header_keys($headers);
            if (isset($normalizedKeys['content-encoding'])) {
                $encoding = $headers[$normalizedKeys['content-encoding']];
                if ($encoding[0] === 'gzip' || $encoding[0] === 'deflate') {
                    $stream = new \UpsProVendor\GuzzleHttp\Psr7\InflateStream(\UpsProVendor\GuzzleHttp\Psr7\stream_for($stream));
    ...
    private function drain(\Psr\Http\Message\StreamInterface $source, \Psr\Http\Message\StreamInterface $sink, $contentLength)
    {
        ...
        \UpsProVendor\GuzzleHttp\Psr7\copy_to_stream($source, $sink, \strlen($contentLength) > 0 && (int) $contentLength > 0 ? (int) $contentLength : -1);
        $sink->seek(0);
        $source->close();
        return $sink;
    }
        ...
        if (isset($options['force_ip_resolve']) && !\filter_var($uri->getHost(), \FILTER_VALIDATE_IP)) {
            if ('v4' === $options['force_ip_resolve']) {
                ...
                if (!isset($records[0]['ip'])) {
                    throw new \UpsProVendor\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv4 address for host '%s'", $uri->getHost()), $request);
                }
                $uri = $uri->withHost($records[0]['ip']);
            } elseif ('v6' === $options['force_ip_resolve']) {
                $records = \dns_get_record($uri->getHost(), \DNS_AAAA);
                if (!isset($records[0]['ipv6'])) {
                    throw new \UpsProVendor\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv6 address for host '%s'", $uri->getHost()), $request);
                }
    ...
    private function add_proxy(\Psr\Http\Message\RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\UpsProVendor\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(\Psr\Http\Message\RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \UpsProVendor\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(\Psr\Http\Message\RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \UpsProVendor\GuzzleHttp\debug_resource($value);
