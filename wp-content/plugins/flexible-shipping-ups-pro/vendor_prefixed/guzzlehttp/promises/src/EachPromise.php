 ...
 */
class EachPromise implements \UpsProVendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    ...
    {
        $this->iterable = \UpsProVendor\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \UpsProVendor\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\UpsProVendor\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \UpsProVendor\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
