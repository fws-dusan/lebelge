...
namespace UpsProVendor\WPDesk\Plugin\Flow\Initialization\Simple;

use UpsProVendor\WPDesk\Helper\HelperRemover;
use UpsProVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use UpsProVendor\WPDesk\License\PluginRegistrator;
use UpsProVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use UpsProVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use UpsProVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use UpsProVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use UpsProVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
use UpsProVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin;
/**
 ...
 */
class SimplePaidStrategy implements \UpsProVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    /** @var SlimPlugin */
    private $plugin;
    public function __construct(\UpsProVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\UpsProVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\UpsProVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \UpsProVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \UpsProVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \UpsProVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \UpsProVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \UpsProVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
