...
if (0 === \intval($days_to_arrival_date)) {
    \_e('(Delivery Days: 0 days)', 'flexible-shipping-ups-pro');
    // WPCS: XSS ok.
} else {
    // Translators: time in transit.
    echo \sprintf(\_n('(Delivery Days: %1$d day)', '(Delivery Days: %1$d days)', $days_to_arrival_date, 'flexible-shipping-ups-pro'), $days_to_arrival_date);
    // WPCS: XSS ok.
}
?>
</div>
