...
namespace UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback;
 ...
 */
class FallbackRateMethod implements \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod
{
    ...
    public function handle_rates(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher $logger, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $metadata_builder, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder $shipment_builder)
    {
        if (empty($method->rates) && !empty('yes' === $method->get_option(self::FIELD_ENABLE_FALLBACK, 'no'))) {
            $fallback_reason = \__('There was no valid services available.', 'flexible-shipping-ups-pro');
            if ($logger->was_error()) {
                $fallback_reason = $logger->get_last_error_message();
            }
            $logger->info(\sprintf(\__('Fallback rate added with reason: %1$s', 'flexible-shipping-ups-pro'), $fallback_reason));
