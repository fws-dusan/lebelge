 ...
 */
namespace UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints;
 ...
 */
class CheckoutHandler implements \UpsProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     ...
     */
    public function __construct(\UpsProVendor\WPDesk\AbstractShipping\CollectionPointCapability\CollectionPointsProvider $collection_points_provider, $method_id, \UpsProVendor\WPDesk\View\Renderer\Renderer $renderer, $field_label, $unavailable_points_label, $field_description, $select_field_available = \false)
    {
    ...
    public function force_shipping_recalculation_on_collection_point_change($unparsed_post_data)
    {
        ...
        $checkout_field_name = \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutField::prepare_field_name_from_shipping_method_id($this->method_id);
    ...
    {
        $address = (new \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutAddress($this->get_request(), $destination))->prepare_address();
    ...
    private function should_show_collection_point()
    {
        ...
        foreach ($packages as $i => $package) {
            ...
            if (isset($package['rates'][$chosen_method])) {
                ...
                if (isset($shipping_rate_meta_data[\UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder::COLLECTION_POINT]) && $shipping_rate_meta_data[\UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder::COLLECTION_POINT] === \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder::YES) {
    ...
    {
        $address = (new \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutAddress($this->get_request()))->prepare_address();
    ...
    public function maybe_display_collection_points_field()
    {
        if ($this->should_show_collection_point()) {
            $field_name = \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutField::prepare_field_name_from_shipping_method_id($this->method_id);
            try {
                $nearest_collection_points = $this->get_collection_points_nearest_to_destination_address();
                $selected_collection_point = $this->get_selected_collection_point($nearest_collection_points, $field_name);
                if ($this->select_field_available) {
                    $checkout_field_class = \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutSelectField::class;
                } else {
                    $checkout_field_class = \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutHtmlField::class;
                }
