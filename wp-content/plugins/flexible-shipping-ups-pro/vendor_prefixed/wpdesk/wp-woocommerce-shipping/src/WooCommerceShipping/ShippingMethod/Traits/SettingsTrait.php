...
use UpsProVendor\WPDesk\WooCommerceShipping\ApiStatus\ApiStatusSettingsDefinitionDecorator;
...
use UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields;
...
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping;
...
trait SettingsTrait
{
     ...
     */
    protected function get_settings_definition_from_service(\UpsProVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
        ...
        if ($shipping_service instanceof \UpsProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanTestSettings) {
            $settings_definitions = new \UpsProVendor\WPDesk\WooCommerceShipping\ApiStatus\ApiStatusSettingsDefinitionDecorator($settings_definitions, $shipping_service->get_field_before_api_status_field(), $plugin_shipping_decisions->get_field_api_status_ajax(), $shipping_service->get_unique_id());
        }
     ...
     * @return array
     */
    private function get_form_fields_from_shipping_service(\UpsProVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
    ...
    public function generate_settings_html($form_fields = array(), $echo = \true)
    {
        ...
        $settings = new \UpsProVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray($this->settings + $this->instance_settings);
        $html = '';
    ...
    private function prepare_custom_field_types($fields)
    {
        ...
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $fields = $this->replace_handling_fees_field_if_exists($fields);
        }
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin) {
            $custom_origin_fields = new \UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields();
            $fields = $custom_origin_fields->replace_fallback_field_if_exists($fields);
        }
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping) {
            $free_shipping_fields = new \UpsProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShippingFields();
    ...
    protected function create_fields_factory()
    {
        if ($this->fields_factory === null) {
            $this->fields_factory = new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\MethodFieldsFactory();
        }
    ...
    private function replace_fallback_field_if_exists($settings)
    {
        ...
        foreach ($settings as $key => $field) {
            if ($field['type'] === \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_TYPE_FALLBACK) {
                $new_settings[\UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_ENABLE_FALLBACK] = ['title' => \__('Fallback', 'flexible-shipping-ups-pro'), 'type' => 'checkbox', 'label' => \__('Enable fallback', 'flexible-shipping-ups-pro'), 'description' => \__('Enable to offer flat rate cost for shipping so that the user can still checkout, if API returns no matching rates.', 'flexible-shipping-ups-pro'), 'desc_tip' => \true, 'default' => 'no'];
                $new_settings[\UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_FALLBACK_COST] = ['title' => \__('Fallback Cost', 'flexible-shipping-ups-pro'), 'type' => 'price', 'required' => \true, 'description' => \__('Enter a numeric value with no currency symbols.', 'flexible-shipping-ups-pro'), 'desc_tip' => \true, 'default' => ''];
            } else {
    ...
    private function replace_handling_fees_field_if_exists($settings)
    {
        $new_settings = [];
        foreach ($settings as $key => $field) {
            if ($field['type'] === \UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::FIELD_TYPE) {
                $field_handling_fees = new \UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees();
    ...
    {
        foreach ($settings as $key => $field) {
            if (isset($field['type']) && \UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::FIELD_TYPE === $field['type']) {
                $settings[$key]['sanitize_callback'] = [\UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::class, 'sanitize'];
            }
        }
        return $settings;
    }
    ...
    public function admin_options()
    {
        ...
        /** @TODO: move to custom field & field footer. */
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $price_adjustment_type_field = $settings_prefix . '_' . \UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_TYPE;
            $price_adjustment_value_field = $settings_prefix . '_' . \UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_VALUE;
            $price_adjustment_type_none = \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone::ADJUSTMENT_TYPE;
