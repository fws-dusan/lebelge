...
use UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustment;
...
trait HandlingFeesTrait
{
    ...
    {
        if ($price_adjustment_type === \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone::ADJUSTMENT_TYPE) {
            return new \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone();
        } elseif ($price_adjustment_type === \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentFixed::ADJUSTMENT_TYPE) {
            return new \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentFixed($price_adjustment_value, \wc_get_rounding_precision());
        } elseif ($price_adjustment_type === \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentPercentage::ADJUSTMENT_TYPE) {
            return new \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentPercentage($price_adjustment_value, \wc_get_rounding_precision());
        }
    ...
    public function apply_handling_fees_if_enabled($price)
    {
        if ($this->should_apply_handling_fees()) {
            $price_adjustment_handler = $this->create_price_adjustment_handler($this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_TYPE, \UpsProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone::ADJUSTMENT_TYPE), \floatval($this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_VALUE, '0')));
