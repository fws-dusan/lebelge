...
use UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CollectionPointFormatter;
use UpsProVendor\WPDesk\WooCommerceShipping\EstimatedDelivery\EstimatedDeliveryMetaDataBuilder;
use UpsProVendor\WPDesk\WooCommerceShipping\OrderMetaData\PackedPackagesMetaDataBuilder;
...
class WooCommerceShippingMetaDataBuilder
{
     ...
     */
    public function __construct(\UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod $shipping_method)
    {
    ...
    private function create_meta_data($rate, $collection_point = self::NO, $shipment = null)
    {
        ...
        if (isset($rate)) {
            ...
            if ($this->shipping_method instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasEstimatedDeliveryDates) {
    ...
    private function append_collection_point_data(array $meta_data, $collection_point)
    {
        ...
        $collection_point_formatter = new \UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CollectionPointFormatter();
     ...
     * @return array
     */
    private function append_packages_meta_data_if_packed(array $meta_data, \UpsProVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment)
    {
        if ($shipment->packed) {
            $meta_data_builder = new \UpsProVendor\WPDesk\WooCommerceShipping\OrderMetaData\PackedPackagesMetaDataBuilder($shipment);
     ...
     * @return array
     */
    private function append_delivery_dates_if_supported_and_exists(array $meta_data, \UpsProVendor\WPDesk\AbstractShipping\Rate\SingleRate $rate)
    {
        $metadata_builder = new \UpsProVendor\WPDesk\WooCommerceShipping\EstimatedDelivery\EstimatedDeliveryMetaDataBuilder($this->shipping_method);
