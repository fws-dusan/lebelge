...
use UpsProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShipping;
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\AddressProvider;
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\CustomOriginAddressSender;
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressReceiver;
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressSender;
...
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher;
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod;
/**
...
trait RateWithRateMethodsTrait
{
    ...
    {
        $origin_country = \explode(':', $this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_COUNTRY, ''));
        return new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\CustomOriginAddressSender($this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_ADDRESS, ''), '', $this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_CITY, ''), $this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_POSTCODE, ''), isset($origin_country[0]) ? $origin_country[0] : '', isset($origin_country[1]) ? $origin_country[1] : '');
    }
    /**
     * Is custom origin?
     *
    ...
    {
        return 'yes' === $this->get_option(\UpsProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::CUSTOM_ORIGIN, 'no');
    }
    /**
    ...
    {
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin && $this->is_custom_origin()) {
            return $this->create_sender_address_from_custom_origin();
        }
        return new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressSender();
    }
    /**
    ...
    protected function create_shipping_builder(array $package)
    {
        if ($this->shipping_builder === null) {
            $this->shipping_builder = new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder();
        }
        ...
        $this->shipping_builder->set_receiver_address(new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressReceiver($package));
     ...
     */
    private function add_rate_method(\UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod $method)
    {
     ...
     */
    private function handle_rating_using_methods(\Psr\Log\LoggerInterface $logger, \UpsProVendor\WPDesk\AbstractShipping\ShippingService $service, array $package, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $metadata_builder)
    {
        $loggerWithErrorCatching = new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher($logger);
    ...
    public function add_rate($args = array())
    {
        ...
        if ($this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasEstimatedDeliveryDates) {
        ...
        if ($cart instanceof \WC_Cart && $this instanceof \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping) {
            $free_shipping = new \UpsProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShipping($this, $logger);
