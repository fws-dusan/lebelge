...
namespace UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Standard;
 ...
 */
class StandardServiceRateMethod implements \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod
{
    /** @var CanRate */
     ...
     */
    public function __construct(\UpsProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRate $service)
    {
    ...
    public function handle_rates(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher $logger, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $metadata_builder, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder $shipment_builder)
    {
        try {
            ...
            if ($rates_count === \count($method->rates)) {
                $logger->info(\__('No rates added from standard service rates!', 'flexible-shipping-ups-pro'));
            }
        } catch (\Exception $e) {
     ...
     */
    private function add_rates(\UpsProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRate $service, \WC_Shipping_Method $method, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $meta_data_builder, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder $shipping_builder)
    {
        $service_id = $method->id;
        $service_settings = \apply_filters("{$service_id}_settings_before_rate", new \UpsProVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray($method->settings + $method->instance_settings), $method);
     ...
     * @param WooCommerceShippingMetaDataBuilder $meta_data_builder Meta data builder.
     */
    private function add_rates_from_response(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment, \UpsProVendor\WPDesk\AbstractShipping\Rate\ShipmentRating $shipment_ratings, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $meta_data_builder)
    {
