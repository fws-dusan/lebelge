...
namespace UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\CollectionPoint;
 ...
 */
class CollectionPointRateMethod implements \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod
{
     ...
     */
    public function __construct(\UpsProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRateToCollectionPoint $service)
    {
     ...
     */
    public static function set_collection_points_checkout_handler(\UpsProVendor\WPDesk\WooCommerceShipping\CollectionPoints\CheckoutHandler $collection_points_checkout_handler)
    {
    ...
    public function handle_rates(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher $logger, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $metadata_builder, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder $shipment_builder)
    {
        try {
            ...
            if ($rates_count === \count($method->rates)) {
                $logger->info(\__('No rates added from collection point rates!', 'flexible-shipping-ups-pro'));
            }
        } catch (\UpsProVendor\WPDesk\AbstractShipping\Exception\CollectionPointNotFoundException $cpe) {
            $logger->error(\__('Collection point not found.', 'flexible-shipping-ups-pro'));
        } catch (\Exception $e) {
     ...
     */
    private function add_rates_to_collection_point(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRateToCollectionPoint $service, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $meta_data_builder, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder $shipment_builder)
    {
        $service_id = $method->id;
        $service_settings = \apply_filters("{$service_id}_settings_before_rate", new \UpsProVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray($method->settings + $method->instance_settings));
        if ($service->is_rate_to_collection_point_enabled($service_settings)) {
            ...
            if (null !== $collection_point) {
                ...
                throw new \UpsProVendor\WPDesk\AbstractShipping\Exception\CollectionPointNotFoundException();
            }
        }
    }
    /**
    ...
    protected function get_collection_point_for_rates(array $package, $method)
    {
        ...
        } catch (\UpsProVendor\WPDesk\AbstractShipping\Exception\CollectionPointNotFoundException $e) {
     ...
     * @param WooCommerceShippingMetaDataBuilder $meta_data_builder Meta data builder.
     */
    private function add_rates_to_collection_point_from_response(\WC_Shipping_Method $method, \UpsProVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment, \UpsProVendor\WPDesk\AbstractShipping\Rate\ShipmentRating $shipment_ratings, $collection_point, \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $meta_data_builder)
    {
