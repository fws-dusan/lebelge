...
namespace UpsProVendor\WPDesk\Logger\WC;

use UpsProVendor\Monolog\Logger;
use UpsProVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured;
/**
...
class WooCommerceCapture
{
    ...
    public function __construct(\UpsProVendor\Monolog\Logger $monolog)
    {
    ...
    private function prepareCaptureHookCallable()
    {
        ...
        if ($this->captureHookFunction === null) {
            $this->captureHookFunction = function () use($monolog) {
                return new \UpsProVendor\WPDesk\Logger\WC\WooCommerceMonologPlugin($monolog, $this->originalWCLogger);
            };
            $this->monolog->pushHandler(new \UpsProVendor\WPDesk\Logger\WC\WooCommerceHandler($this->originalWCLogger));
        }
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            if ($this->isCaptured) {
                throw new \UpsProVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured('Try to free wc logger first.');
            }
