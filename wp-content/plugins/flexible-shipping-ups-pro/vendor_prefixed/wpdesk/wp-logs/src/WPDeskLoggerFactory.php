...
use LogicException;
use UpsProVendor\Monolog\Handler\NullHandler;
use UpsProVendor\Monolog\Logger;
use UpsProVendor\Monolog\Registry;
use UpsProVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use UpsProVendor\WPDesk\Logger\WC\WooCommerceCapture;
use UpsProVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \UpsProVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\UpsProVendor\Monolog\Registry::hasLogger($name)) {
            \UpsProVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\UpsProVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \UpsProVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \UpsProVendor\Monolog\Logger($name);
        }
        if (\UpsProVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \UpsProVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\UpsProVendor\Monolog\Logger $logger)
    {
        if (!\defined('UpsProVendor\\WC_LOG_THRESHOLD')) {
            \define('UpsProVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \UpsProVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \UpsProVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \UpsProVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \UpsProVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\UpsProVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \UpsProVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \UpsProVendor\Monolog\Registry::hasLogger($name);
    }
}
