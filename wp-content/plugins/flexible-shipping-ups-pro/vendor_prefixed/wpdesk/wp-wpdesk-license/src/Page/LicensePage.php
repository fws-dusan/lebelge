...
use UpsProVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \UpsProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\UpsProVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    {
        \add_submenu_page('wpdesk-helper', \__('Subscriptions', 'flexible-shipping-ups-pro'), \__('Subscriptions', 'flexible-shipping-ups-pro'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('UpsProVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \UpsProVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
