...
use UpsProVendor\WPDesk\License\ServerAddressRepository;
use UpsProVendor\WPDesk_API_Manager_With_Update_Flag;
/**
 * Can activate plugin license.
 *
 * @package WPDesk\License\Page\License\Action
 */
class LicenseActivation implements \UpsProVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \UpsProVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        \add_settings_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-shipping-ups-pro'), 'error');
    }
    /**
    ...
    {
        \add_settings_error('activate_text', 'activate_msg', \__('Plugin activated. ', 'flexible-shipping-ups-pro') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \UpsProVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
