...
namespace UpsProVendor\WpDesk\WooCommerce\ShippingMethod;
...
class PackerBoxesFactory
{
     ...
     * @return string
     */
    private static function custom_box_name(\UpsProVendor\WpDesk\WooCommerce\ShippingMethod\SettingsBox $setting_box)
    {
        return \sprintf(\__('Custom (%1$sx%2$sx%3$s/%4$s)', 'flexible-shipping-ups-pro'), self::format_decimal($setting_box->get_length()), self::format_decimal($setting_box->get_width()), self::format_decimal($setting_box->get_height()), self::format_decimal($setting_box->get_max_weight()));
    }
    /**
     ...
     * @return SettingsBox
     */
    private static function prepare_from_built_in_box(\UpsProVendor\WpDesk\WooCommerce\ShippingMethod\SettingsBox $setting_box, array $built_in_boxes)
    {
        $found_built_in_box = \array_reduce($built_in_boxes, function ($carry, \UpsProVendor\WPDesk\Packer\Box $box) use($setting_box) {
    ...
    public static function create_packer_boxes_from_settings($boxes_settings, array $built_in_boxes)
    {
        ...
        foreach (\json_decode($boxes_settings, \true) as $box_setting) {
            $setting_box = \UpsProVendor\WpDesk\WooCommerce\ShippingMethod\SettingsBox::create_from_array($box_setting);
            $setting_box = static::prepare_from_built_in_box($setting_box, $built_in_boxes);
            $packer_boxes[] = new \UpsProVendor\WPDesk\Packer\Box\BoxImplementation($setting_box->get_length() - $setting_box->get_padding(), $setting_box->get_width() - $setting_box->get_padding(), $setting_box->get_height() - $setting_box->get_padding(), $setting_box->get_box_weight(), $setting_box->get_max_weight(), $setting_box->get_code(), $setting_box->get_name(), array('name' => $setting_box->get_name(), 'box' => $setting_box));
        }
        return $packer_boxes;
    }
}
