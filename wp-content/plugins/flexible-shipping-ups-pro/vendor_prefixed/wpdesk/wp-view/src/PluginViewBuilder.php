...
namespace UpsProVendor\WPDesk\View;
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \UpsProVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \UpsProVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \UpsProVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \UpsProVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
