 ...
 * UPS Shipping Method.
 */
class UpsShippingMethod extends \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod implements \UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping
{
    /**
     * Supports.
     *
     ...
     */
    public static function set_api_status_ajax_handler(\UpsProVendor\WPDesk\WooCommerceShipping\CustomFields\ApiStatus\FieldApiStatusAjax $api_status_ajax_handler)
    {
    ...
    private function prepare_description()
    {
        ...
        return \sprintf(
            ...
            \__('The UPS extension obtains rates dynamically from the UPS API during cart/checkout. %1$sRefer to the instruction manual →%2$s', 'flexible-shipping-ups-pro'),
    ...
    {
        $ups_settings_definition = new \UpsProVendor\WPDesk\WooCommerceShipping\Ups\UpsSettingsDefinitionWooCommerce($this->form_fields);
    ...
    {
        return new \UpsProVendor\WPDesk\WooCommerceShipping\Ups\UpsMetaDataBuilder($this);
    }
    /**
    ...
    {
        $this->instance_form_fields[\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::SERVICES]['options'] = \UpsProVendor\WPDesk\UpsShippingService\UpsServices::get_services_for_country($this->get_origin_country_code());
    }
    /**
    ...
    {
        return 'yes' === $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::CUSTOM_ORIGIN, 'no');
    }
    /**
    ...
    public function get_origin_country_code()
    {
        ...
        if ($this->is_custom_origin()) {
            $country_state_code = \explode(':', $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_COUNTRY, ''));
    ...
    private function get_available_services($get_current_services = \true)
    {
        $country_code = '';
        if ($this->is_custom_origin()) {
            $country_codes = \explode(':', $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_COUNTRY, ''));
        ...
        $services_available = \UpsProVendor\WPDesk\UpsShippingService\UpsServices::get_services_for_country($country_code);
        $services = array();
        if ($get_current_services) {
            $current_services = $this->get_instance_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::SERVICES, array());
    ...
    {
        if ($this->is_custom_origin()) {
            $origin_country = \explode(':', $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_COUNTRY, ''));
            return new \UpsProVendor\WPDesk\WooCommerceShipping\ShippingBuilder\CustomOriginAddressSender($this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_ADDRESS, ''), '', $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_CITY, ''), $this->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_POSTCODE, ''), isset($origin_country[0]) ? $origin_country[0] : '', isset($origin_country[1]) ? $origin_country[1] : '');
        }
