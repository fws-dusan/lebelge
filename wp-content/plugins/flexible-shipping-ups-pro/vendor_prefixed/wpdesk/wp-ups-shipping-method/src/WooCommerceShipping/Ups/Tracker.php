 ...
 */
class Tracker implements \UpsProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     * @return array
     */
    private function prepare_default_plugin_data(\UpsProVendor\WPDesk\WooCommerceShipping\Ups\UpsShippingMethod $flexible_shipping_ups)
    {
        if ('yes' === $flexible_shipping_ups->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::CUSTOM_ORIGIN, 'no')) {
            $ups_origin_country = \explode(':', $flexible_shipping_ups->get_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ORIGIN_COUNTRY, ''));
        } else {
     ...
     * @return array
     */
    private function append_data_for_shipping_method(array $plugin_data, \WC_Shipping_Zone $zone, \UpsProVendor\WPDesk\WooCommerceShipping\Ups\UpsShippingMethod $shipping_method)
    {
        ...
        if (self::OPTION_VALUE_YES === $shipping_method->get_instance_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::NEGOTIATED_RATES, self::OPTION_VALUE_NO)) {
            $plugin_data['negotiated_rates']++;
        }
        if (self::OPTION_VALUE_YES === $shipping_method->get_instance_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::INSURANCE, self::OPTION_VALUE_NO)) {
            $plugin_data['insurance_option']++;
        }
        if (self::OPTION_VALUE_YES === $shipping_method->get_instance_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::FALLBACK, self::OPTION_VALUE_NO)) {
            $plugin_data['fallback']++;
        }
        $access_point_option = $shipping_method->get_instance_option(\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ACCESS_POINT, \UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::DO_NOT_ADD_ACCESS_POINTS_TO_RATES);
        if (\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ADD_ACCESS_POINTS_TO_RATES === $access_point_option) {
            $plugin_data['access_point']++;
        }
        if (\UpsProVendor\WPDesk\UpsShippingService\UpsSettingsDefinition::ADD_ONLY_ACCESS_POINTS_TO_RATES === $access_point_option) {
            $plugin_data['access_point_only']++;
        }
        //Count free shipping uses.
        if (self::OPTION_VALUE_YES === $shipping_method->get_instance_option(\UpsProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShippingFields::FIELD_STATUS, self::OPTION_VALUE_NO)) {
        ...
        if (isset($shipping_methods['flexible_shipping_ups'])) {
            ...
            foreach ($shipping_zones as $zone_data) {
                ...
                foreach ($shipping_methods as $shipping_method) {
                    if ('flexible_shipping_ups' === $shipping_method->id && $shipping_method instanceof \UpsProVendor\WPDesk\WooCommerceShipping\Ups\UpsShippingMethod) {
