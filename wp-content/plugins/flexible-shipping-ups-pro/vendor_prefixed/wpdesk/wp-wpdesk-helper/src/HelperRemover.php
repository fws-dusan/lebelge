...
use UpsProVendor\WPDesk\PluginBuilder\Storage\Exception\ClassNotExists;
 ...
 */
class HelperRemover implements \UpsProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    private function remove_helper_updates()
    {
        ...
        \add_action('plugins_loaded', function () {
            \remove_action('plugins_loaded', [$this->get_helper_instance(), 'init_helper_plugins'], \UpsProVendor\WPDesk\Helper\Integration\LicenseIntegration::PRIORITY_HELPER_UPDATE);
        });
    }
    /**
     * @return void
    ...
    {
        $storage = new \UpsProVendor\WPDesk\PluginBuilder\Storage\StorageFactory();
        try {
            return $storage->create_storage()->get_from_storage('WPDesk_Helper');
        } catch (\UpsProVendor\WPDesk\PluginBuilder\Storage\Exception\ClassNotExists $e) {
            return null;
        }
    }
}
