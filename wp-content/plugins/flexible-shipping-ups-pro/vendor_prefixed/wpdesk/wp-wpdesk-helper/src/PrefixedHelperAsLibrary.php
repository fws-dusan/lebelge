...
namespace UpsProVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use UpsProVendor\WPDesk\Helper\Debug\LibraryDebug;
use UpsProVendor\WPDesk\Helper\Integration\LicenseIntegration;
use UpsProVendor\WPDesk\Helper\Integration\LogsIntegration;
use UpsProVendor\WPDesk\Helper\Integration\SettingsIntegration;
use UpsProVendor\WPDesk\Helper\Integration\TrackerIntegration;
use UpsProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use UpsProVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \UpsProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const LIBRARY_TEXT_DOMAIN = 'flexible-shipping-ups-pro';
    ...
    {
        $helper_info = new \UpsProVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \UpsProVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \UpsProVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \UpsProVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \UpsProVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \UpsProVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \UpsProVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \UpsProVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \UpsProVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \UpsProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
