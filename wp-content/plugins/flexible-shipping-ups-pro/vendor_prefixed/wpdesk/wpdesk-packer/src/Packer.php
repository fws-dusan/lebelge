...
use UpsProVendor\WPDesk\Packer\Exception\NoItemsException;
/**
...
class Packer
{
     ...
     * @param Item $item
     */
    public function add_item(\UpsProVendor\WPDesk\Packer\Item $item)
    {
        $this->items[] = $item;
    }
    /**
     * @param Box $box
     */
    public function add_box(\UpsProVendor\WPDesk\Packer\Box $box)
    {
        $this->boxes[] = $box;
    }
    /**
    ...
    public function pack()
    {
        if (\sizeof($this->items) === 0) {
            throw new \UpsProVendor\WPDesk\Packer\Exception\NoItemsException('No items to pack!');
        }
        $this->packages = [];
    ...
    private function find_best_packed_box()
    {
        ...
        foreach ($this->boxes as $box) {
            $packages[] = new \UpsProVendor\WPDesk\Packer\PackedBox($box, $this->items);
        }
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\UpsProVendor\WPDesk\Packer\Box $a, \UpsProVendor\WPDesk\Packer\Box $b) {
    ...
    {
        if (!empty($sort)) {
            \uasort($sort, static function (\UpsProVendor\WPDesk\Packer\Item $a, \UpsProVendor\WPDesk\Packer\Item $b) {
                if ($a->get_volume() === $b->get_volume()) {
