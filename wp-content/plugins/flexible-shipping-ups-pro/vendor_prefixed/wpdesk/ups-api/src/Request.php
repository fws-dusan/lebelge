...
use DOMDocument;
use Exception;
use UpsProVendor\GuzzleHttp\Client as Guzzle;
use Psr\Log\LoggerAwareInterface;
use Psr\Log\LoggerInterface;
use Psr\Log\NullLogger;
use SimpleXMLElement;
use UpsProVendor\Ups\Exception\InvalidResponseException;
use UpsProVendor\Ups\Exception\RequestException;
class Request implements \UpsProVendor\Ups\RequestInterface, \Psr\Log\LoggerAwareInterface
{
    /**
     * @var string
     */
    ...
    {
        $this->client = new \UpsProVendor\GuzzleHttp\Client();
    }
    /**
     * Send request to UPS.
     *
    ...
    public function request($access, $request, $endpointurl)
    {
        ...
        try {
            ...
            if ($response->getStatusCode() === 200) {
                ...
                if ('' === \trim($body)) {
                    throw new \UpsProVendor\Ups\Exception\InvalidResponseException('Failure: response is an empty string.');
                } else {
                    $xml = new \SimpleXMLElement($body);
                    if (isset($xml->Response) && isset($xml->Response->ResponseStatusCode)) {
                        if ($xml->Response->ResponseStatusCode == 1) {
                            $responseInstance = new \UpsProVendor\Ups\Response();
                            ...
                            throw new \UpsProVendor\Ups\Exception\InvalidResponseException('Failure: ' . $xml->Response->Error->ErrorDescription . ' (' . $xml->Response->Error->ErrorCode . ')', $code);
                        }
                    } else {
                        throw new \UpsProVendor\Ups\Exception\InvalidResponseException('Failure: response is in an unexpected format.');
                    }
                }
            }
        } catch (\UpsProVendor\GuzzleHttp\Exception\TransferException $e) {
            ...
            throw new \UpsProVendor\Ups\Exception\RequestException('Failure: ' . $e->getMessage());
        }
    }
    /**
     * Formats XML.
