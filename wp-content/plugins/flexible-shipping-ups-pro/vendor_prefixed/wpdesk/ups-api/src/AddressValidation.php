...
use UpsProVendor\Ups\Entity\AddressValidationResponse;
/**
 * Address Validation API Wrapper.
 *
 * This functionality is only available in USA, Puerto Rico & Canada.
 */
class AddressValidation extends \UpsProVendor\Ups\Ups
{
    const ENDPOINT = '/XAV';
    /**
     ...
     */
    public function validate(\UpsProVendor\Ups\Entity\Address $address, $requestOption = self::REQUEST_OPTION_ADDRESS_VALIDATION, $maxSuggestion = 15)
    {
        if ($maxSuggestion > 50) {
        ...
        if ($this->useAVResponseObject) {
            ...
            $avResponse = new \UpsProVendor\Ups\Entity\AddressValidationResponse($response, $requestOption);
            return $avResponse;
        }
