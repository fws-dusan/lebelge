...
class SoapRequest implements \UpsProVendor\Ups\RequestInterface, \Psr\Log\LoggerAwareInterface
{
    /**
     * @var string
     */
    ...
    public function request($access, $request, $endpointurl, $operation = null, $wsdl = null)
    {
        ...
        try {
            ...
            if (isset($errorCode[0]) && isset($errorMsg[0])) {
                ...
                throw new \UpsProVendor\Ups\Exception\InvalidResponseException('Failure: ' . (string) $errorMsg[0] . ' (' . (string) $errorCode[0] . ')');
            } else {
                $this->logger->alert($e->getMessage(), ['id' => $id, 'endpointurl' => $this->getEndpointUrl()]);
                throw new \UpsProVendor\Ups\Exception\InvalidResponseException('Cannot parse error from UPS: ' . $e->getMessage(), $e->getCode(), $e);
            }
        }
    }
    /**
