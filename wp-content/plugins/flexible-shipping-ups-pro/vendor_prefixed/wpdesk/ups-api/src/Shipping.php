...
use UpsProVendor\Ups\Entity\ShipmentRequestLabelSpecification;
use UpsProVendor\Ups\Entity\ShipmentRequestReceiptSpecification;
/**
 * Package Shipping API Wrapper
 * Based on UPS Developer Guide, dated: 31 Dec 2012.
 */
class Shipping extends \UpsProVendor\Ups\Ups
{
     ...
     */
    public function __construct($accessKey = null, $userId = null, $password = null, $useIntegration = \false, \UpsProVendor\Ups\RequestInterface $request = null, \Psr\Log\LoggerInterface $logger = null)
    {
        if (null !== $request) {
     ...
     * @return \stdClass
     */
    public function confirm($validation, \UpsProVendor\Ups\Entity\Shipment $shipment, \UpsProVendor\Ups\Entity\ShipmentRequestLabelSpecification $labelSpec = null, \UpsProVendor\Ups\Entity\ShipmentRequestReceiptSpecification $receiptSpec = null)
    {
     ...
     * @return string
     */
    private function createConfirmRequest($validation, \UpsProVendor\Ups\Entity\Shipment $shipment, \UpsProVendor\Ups\Entity\ShipmentRequestLabelSpecification $labelSpec = null, \UpsProVendor\Ups\Entity\ShipmentRequestReceiptSpecification $receiptSpec = null)
    {
        $xml = new \DOMDocument();
        ...
        if ($shipment->getPaymentInformation()) {
            ...
            for ($shipmentChargeRec = 1; $shipmentChargeRec <= 2; $shipmentChargeRec++) {
                if ($shipmentChargeRec === 1) {
                    ...
                    $node->appendChild($xml->createElement('Type', \UpsProVendor\Ups\Entity\ShipmentCharge::SHIPMENT_CHARGE_TYPE_TRANSPORTATION));
                } else {
                    ...
                    $node = $paymentNode->appendChild($xml->createElement('ShipmentCharge'));
                    $node->appendChild($xml->createElement('Type', \UpsProVendor\Ups\Entity\ShipmentCharge::SHIPMENT_CHARGE_TYPE_DUTIES));
                }
    ...
    public function getRequest()
    {
        if (null === $this->request) {
            $this->request = new \UpsProVendor\Ups\Request($this->logger);
        }
        return $this->request;
    }
    /**
     ...
     * @return $this
     */
    public function setRequest(\UpsProVendor\Ups\RequestInterface $request)
    {
     ...
     * @return $this
     */
    public function setResponse(\UpsProVendor\Ups\ResponseInterface $response)
    {
     ...
     * @return DOMNode
     */
    private function compileReceiptSpecificationNode(\UpsProVendor\Ups\Entity\ShipmentRequestReceiptSpecification $receiptSpec)
    {
        $xml = new \DOMDocument();
        $xml->formatOutput = \true;
     ...
     * @return DOMNode
     */
    private function compileLabelSpecificationNode(\UpsProVendor\Ups\Entity\ShipmentRequestLabelSpecification $labelSpec)
    {
        $xml = new \DOMDocument();
        $xml->formatOutput = \true;
        ...
        if ($labelSpec->getPrintMethodCode() == \UpsProVendor\Ups\Entity\ShipmentRequestLabelSpecification::IMG_FORMAT_CODE_GIF) {
